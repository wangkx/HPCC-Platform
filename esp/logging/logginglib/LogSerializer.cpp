...
void CLogSerializer::Append(const char* GUID, const char* Data, CLogContentInFile* contentInFile)
{
    ...
    if (contentInFile)
    {
        contentInFile->setFileName(m_file->queryFilename());
        contentInFile->setPos(m_file->size());
        contentInFile->setSize(len);
    }
    m_ItemCount++;
    fileSize += len;
