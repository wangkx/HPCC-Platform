     ...
     bool firstProcess, IArrayOf<IEspSWRunInfo>& processArray, IEspComponentInfo* pComponentInfo)
{
    ...
    //Some component (ex. dropzone) has no process type since it has not a process and does not have a PID.
    ...
    if (!dependencyDown && (!isEmptyString(procPID) || isEmptyString(procType)))
    {
    ...
    else if (strieq(procType, "FTSlaveProcess"))
    {
        pComponentInfo->setCondition( 1 );
        pComponentInfo->setState(5);
    }
