...
class CWSESPControlEx : public CWSESPControl
{
    ...
    StringAttr espProcess, authDomain;
    IEspContainer* m_container;

    const char* readSessionState(int st);
    const char* readSessionTimeStamp(int t, StringBuffer& str);
    float readSessionTimeoutMin(int lastAccessed);
    IEspSession* setSessionInfo(const char* sessionID, IPropertyTree* espSessionTree, IEspSession* session);
