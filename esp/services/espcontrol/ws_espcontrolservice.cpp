...
bool CWSESPControlEx::onSessionInfo(IEspContext& context, IEspSessionInfoRequest& req, IEspSessionInfoResponse& resp)
{
    try
    {
        ...
        try
        {
#define Lock_SDS_120_sec
#ifdef Lock_SDS_120_sec
            Owned<IRemoteConnection> conn = querySDS().connect("Sessions/Process[@name='myesp']", myProcessSession(), RTM_LOCK_WRITE, 120000);
            Owned<IRemoteConnection> conn1 = querySDS().connect("Sessions/Process[@name='myesp']", myProcessSession(), RTM_LOCK_WRITE, 120000);
            return true;
#else
