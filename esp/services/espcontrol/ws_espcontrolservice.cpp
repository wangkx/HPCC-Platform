...
float CWSESPControlEx::readSessionTimeoutMin(int lastAccessed)
{
...
IEspSession* CWSESPControlEx::setSessionInfo(const char* sessionID, IPropertyTree* espSessionTree, IEspSession* session)
{
    ...
    float timeout = readSessionTimeoutMin(lastAccessed);
    if (timeout < 0.0)
...
void CWSESPControlEx::init(IPropertyTree *cfg, const char *process, const char *service)
{
    ...
    sessionTimeoutMinutes = espCFG->getPropInt("@sessionTimeoutMinutes", ESP_SESSION_TIMEOUT);

    const char* domain = espCFG->queryProp("AuthDomains/AuthDomain/@name");
    authDomain.set((domain && *domain)? domain : "default");
}
...
bool CWSESPControlEx::onSessionQuery(IEspContext& context, IEspSessionQueryRequest& req, IEspSessionQueryResponse& resp)
{
    try
    {
        ...
        VStringBuffer xpath("/%s/%s[@name='%s']/%s[@name='%s']", PathSessionRoot, PathSessionProcess, espProcess.get(), PathSessionDomain, authDomain.get());
        ...
        ForEach(*iter)
        {
            ...
            setSessionInfo(sessionTree.queryProp(PropSessionID), &sessionTree, s);
...
bool CWSESPControlEx::onSessionInfo(IEspContext& context, IEspSessionInfoRequest& req, IEspSessionInfoResponse& resp)
{
    try
    {
        ...
        VStringBuffer xpath("/%s/%s[@name='%s']/%s[@name='%s']/%s[%s='%s']", PathSessionRoot, PathSessionProcess, espProcess.get(),
            PathSessionDomain, authDomain.get(), PathSessionSession, PropSessionID, sessionID.str());
        Owned<IRemoteConnection> globalLock = querySDS().connect(xpath.str(), myProcessSession(), RTM_LOCK_READ, SESSION_SDS_LOCK_TIMEOUT);
        if (!globalLock)
            throw MakeStringException(ECLWATCH_INTERNAL_ERROR, "Unable to connect to ESP Session information in dali %s", xpath.str());
        ...
        setSessionInfo(sessionID.str(), espSession, &resp.updateSession());
    }
    catch(IException* e)
    {
        FORWARDEXCEPTION(context, e, ECLWATCH_INTERNAL_ERROR);
    ...
    try
    {
        ...
        ForEach(*iter)
        {
            ...
            else
            {
                CDateTime timeNow;
                timeNow.setNow();
                timeNow.adjustTime(timeoutMinutes - sessionTimeoutMinutes);
                item.setPropInt64(PropSessionLastAccessed, timeNow.getSimple());
