...
bool CWsWorkunitsEx::onWUCreateZAPInfoFile(IEspContext &context, IEspWUCreateZAPInfoFileRequest &req, IEspWUCreateZAPInfoFileResponse &resp)
{
    try
    {
        ...
        WsWuHelpers::checkAndTrimWorkunit("onWUCreateZAPInfoFile", zapInfoReq.wuid);
        Owned<IWorkUnitFactory> factory = getWorkUnitFactory(context.querySecManager(), context.queryUser());
        ...
        PROGLOG("WUCreateZAPInfoFile(): %s", zapInfoReq.wuid.str());
        ...
        helper.createWUZAPFile(context, cwu, zapInfoReq, zapFileName, zapFileNameWithPath);
        resp.setZAPFileName(zapFileName.str());
    }
    catch(IException* e)
    {
        FORWARDEXCEPTION(context, e,  ECLWATCH_INTERNAL_ERROR);
