...
void CFileSprayEx::init(IPropertyTree *cfg, const char *process, const char *service)
{
    ...
    tpWrapper.getTpDropZones(9999, nullptr, false, allTpDropZones); //version 9999: get all information about dropzone
...
bool CFileSprayEx::checkDropZoneIPAndPath(double clientVersion, const char* dropZoneName, const char* netAddr, const char* path)
{
    ...
    ForEachItemIn(i, allTpDropZones)
    {
        ...
        if (isEmptyString(name))
            continue;
        if (!isEmptyString(dropZoneName) && !streq(dropZoneName, name))
            continue;
...
bool CFileSprayEx::onDropZoneFileSearch(IEspContext &context, IEspDropZoneFileSearchRequest &req, IEspDropZoneFileSearchResponse &resp)
{
    try
    {
        ...
        const char* dropZoneServerReq = readNetAddress(req.getServer(), serverBuffer); //IP or hostname
        ...
        ForEachItemIn(i, allTpDropZones)
        {
            IConstTpDropZone& dropZone = allTpDropZones.item(i);
            const char* name = dropZone.getName();
            if (isEmptyString(name) || !streq(dropZoneName, name))
                continue;
            if (req.getECLWatchVisibleOnly() && !dropZone.getECLWatchVisible())
                continue;
            if (isEmptyString(dropZone.getPath()))
                continue;
            ...
            {
                IConstTpMachine& tpMachine = tpMachines.item(ii);
                const char* netAddress = tpMachine.getNetaddress();
                if (isEmptyString(netAddress) || !strieq(dropZoneServerReq, netAddress))
                    continue;

                IpAddress ipAddress;
                ipAddress.ipset(netAddress);
...
bool CFileSprayEx::onDropZoneFiles(IEspContext &context, IEspDropZoneFilesRequest &req, IEspDropZoneFilesResponse &resp)
{
    try
    {
        ...
        StringBuffer netAddressBuffer;
        const char* netAddress = readNetAddress(req.getNetAddress(), netAddressBuffer);
        ...
        ForEachItemIn(i, allTpDropZones)
        {
            ...
            const char* prefix = dropZone.getPath();
            if (isEmptyString(dropZoneName) || isEmptyString(prefix))
                continue;
            ...
            ForEachItemIn(ii, tpMachines)
            {
                ...
                if (!isEmptyString(netAddress) && strieq(netAddress, netAddr))
...
bool CFileSprayEx::onGetDFUServerQueues(IEspContext &context, IEspGetDFUServerQueuesRequest &req, IEspGetDFUServerQueuesResponse &resp)
{
    try
    {
        ...
        StringArray qlist;
#ifdef _CONTAINERIZED
        qlist.append(m_QueueLabel); // TODO: re-design support for multiple queues (see the comment in init()).
#else
...
StringBuffer& CFileSprayEx::readNetAddress(const char* addressIn, StringBuffer& addressOut)
{
