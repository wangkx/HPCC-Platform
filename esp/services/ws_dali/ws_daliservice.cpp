...
int CWSDaliSoapBindingEx::onGet(CHttpRequest* request, CHttpResponse* response)
{
    try
    {
        checkAccess(request->queryContext());
        checkDaliDetached(true);
...
void CWSDaliSoapBindingEx::checkAccess(IEspContext* context)
{
#ifdef _USE_OPENLDAP
    ...
    if(secmgr && !secmgr->isSuperUser(context->queryUser()))
    {
        ...
        throw MakeStringException(ECLWATCH_SUPER_USER_ACCESS_DENIED, "Failed to access WSDali. Permission denied.");
    }
#endif
}

bool CWSDaliSoapBindingEx::checkDaliDetached(bool throwException)
{
    if (throwException && wsdService->daliDetached)
        throw MakeStringException(ECLWATCH_CANNOT_CONNECT_DALI, "Dali detached.");
    return wsdService->daliDetached;
}
