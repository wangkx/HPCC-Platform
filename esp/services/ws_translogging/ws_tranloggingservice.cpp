...
#include "ws_tranloggingservice.hpp"
...
IEspLogAgent* CWSTransactionLogEx::loadLoggingAgent(const char* name, const char* dll, const char* service, IPropertyTree* cfg)
{
    StringBuffer plugin;
...
void CWSTransactionLogEx::init(IPropertyTree* cfg, const char* process, const char* service)
{
    if (!cfg)
        throw MakeStringException(-1, "Can't initialize CWSTransactionLogEx, cfg is NULL");
    ...
    tankFile.set(serviceCFG->queryProp("TankFile"));
    if (!tankFile.length())
        throw MakeStringException(-1, "Can't initialize CWSTransactionLogEx, @tankFile is NULL");
    ...
    ForEach(*loggingAgentSettings)
    {
        ...
        IUpdateLogThread* logThread = createUpdateLogThread(&loggingAgentTree, service, agentName, tankFile.get(), loggingAgent);
        if(!logThread)
...
bool CWSTransactionLogEx::onGetLogAgentSetting(IEspContext& context, IEspGetLogAgentSettingRequest& req, IEspGetLogAgentSettingResponse& resp)
{
    try
    {
...
bool CWSTransactionLogEx::checkName(const char* name, StringArray& names, bool defaultValue)
{
    if (!names.length())
        return defaultValue;
...
bool CWSTransactionLogEx::onPauseLog(IEspContext& context, IEspPauseLogRequest& req, IEspPauseLogResponse& resp)
{
