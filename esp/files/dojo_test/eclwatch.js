...
require(["dijit", "dojo/_base/xhr", "dojo/dom"]);
				
function refreshContent(url) {
	var tab2 = dijit.byId('mainContent');
	if (tab2.get('selected')) {
		contentNodeId = 'mainContent';
	} else {
		contentNodeId = 'mainContent2';
	}

        var urlString = String(url);
	if (urlString.match('Not')) {
                dojo.byId(contentNodeId).innerHTML = '<html><body><h4>This link will be implemented soon.</h4></body></html>';
		return;
	}
	...
	dojo.xhrGet({
		...
		load: function(newContent) {
			dojo.byId(contentNodeId).innerHTML = newContent;
		},
		// The error handler
		error: function() {
