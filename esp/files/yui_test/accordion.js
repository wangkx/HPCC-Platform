...
(function() {
	...
	YAHOO.widget.AccordionManager = function() {
        ...
        function _onresize ( list ) {
            //params = {};
            if (list) {
	            // closing all the selected items
		        for (i=0; i<list.items.length; i++) {
		          // is the element is not equal to item, or if the item is under an animation...
				  //if ((list.items[i].selected || list.items[i].sliding || params.expand)) {
					if (list.items[i].selected) {
						list.items[i].selected = false;
						_openItem (list.items[i], list);
					} else {
						_closeItem (list.items[i], list);
					}
	            }
	        }
        }
		...
		* @return boolean
		*/
		obj.reset = function ( el) {
		    var list;
		    if ((list = _getList ({el:el}))) {
		        return _onresize (list);
		    }
		};
		/**
		* @method open
