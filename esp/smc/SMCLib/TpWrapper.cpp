...
void CTpWrapper::getContainerServices(double version, const char* serviceType, const char* serviceName, IArrayOf<IConstContainerService>& services)
{
    ...
    ForEach(*itr)
    {
        ...
        Owned<IEspContainerService> svc = createContainerService();
        svc->setName(name);
        svc->setType(type);
