...
void CHttpMessage::setContent(IPropertyTree* ptree)
{
    if (ptree != nullptr)
    {
...
class CHttpMessageIOAdapter : public CInterfaceOf<IIOStream>
{
    ...
    void sendChunk();
public:
    CHttpMessageIOAdapter(CHttpMessage* _response, unsigned _flushThreshold) : response(_response), flushThreshold(_flushThreshold) { }
...
int CHttpMessage::send()
{
    ...
    if (!m_content.length() && m_content_ptree != nullptr)
    {
        CHttpMessageIOAdapter ioa(this, defaultHttpMessageFlushThreshold);
...
StringBuffer& CHttpResponse::constructHeaderBuffer(StringBuffer& headerbuf, bool inclLen)
{
    ...
    if(inclLen && m_content_length > 0)
        headerbuf.append("Content-Length: ").append(m_content_length).append("\r\n");
    else if (!m_content.length() && m_content_ptree != nullptr)
