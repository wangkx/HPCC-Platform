...
public:
...
#ifdef _USE_OPENLDAP
    bool useGefaultAuthPage() { return defaultAuthPage; };
    ...
    void readAuthCookie(CHttpRequest* request, IEspContext* ctx);
    bool readAuthCookie(CHttpRequest* request, StringBuffer& userId, StringBuffer& password);
    espAuthState_ checkUserAuth(CHttpRequest* request, CHttpResponse* response, IEspContext* ctx);
    void notifyUserLogOn(const char* prompt, CHttpResponse* response);
    void handleUserLogOn(CHttpRequest* request, CHttpResponse* response);
    void handleUserLogOut(CHttpResponse* response);
