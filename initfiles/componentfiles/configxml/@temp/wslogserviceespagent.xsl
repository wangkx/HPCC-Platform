...
xmlns:set="http://exslt.org/sets">
    ...
    <xsl:template name="WsLogServiceESPAgent" type="DefaultLoggingAgent">
        ...
        <xsl:if test="not($agentNode/LogDataItem[1]) and not($agentNode/LogInfo[1])">
            <xsl:message terminate="yes">Log Data XPath1 is undefined for <xsl:value-of select="$agentName"/> </xsl:message>
        </xsl:if>
        ...
        <LogAgent name="{$agentName}" type="LogAgent" services="GetTransactionSeed,UpdateLog,GetTransactionID" plugin="wslogserviceespagent">
            ...
            <LogDataXPath>
                <xsl:for-each select="$agentNode/LogDataItem">
                    <LogDataItem name="{current()/@name}" XPath="{current()/@xpath}" default="{current()/@default}"/>
                </xsl:for-each>
                <xsl:for-each select="$agentNode/LogInfo">
                    <LogInfo name="{current()/@name}" default="{current()/@default}" XPath="{current()/@xpath}" multiple="{current()/@multiple}" encode="{current()/@encode}" type="{current()/@type}"/>
                </xsl:for-each>
            </LogDataXPath>
        </LogAgent>
    </xsl:template>
