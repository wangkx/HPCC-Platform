...
void zlib_deflate(MemoryBuffer &mb, const char* inputBuffer, unsigned int inputSize, int compressionLevel, ZlibCompressionType zltype)
{
    ...
    {
        //If we are not yet at the end of the input, then the zlib constant Z_NO_FLUSH will
        //be passed to deflate to indicate that we are still in the middle of the uncompressed data.
        //Z_NO_FLUSH: allow deflate to decide how much data to accumulate before producing output,
    ...
    while (ret == Z_OK)
    {
        //Once the Z_FINISH parameter is provided, deflate() will begin to complete the compressed
        //output stream. However depending on how much output space is provided, deflate() may have
        //to be called several times until it has provided the complete compressed stream, even after
        //it has consumed all of the input. The flush parameter must continue to be Z_FINISH for those
        //subsequent calls.
