...
#define MAXDELAYS 5
static const char *deltaHeader =
        "<CRC>0000000000</CRC><SIZE>0000000000000000</SIZE>"; // fill in later
...
public:
    LinkingCriticalBlock(CheckedCriticalSection &crit, const char *file,
            unsigned line) :
            CheckedCriticalBlock(crit, fakeCritTimeout, file, line) {
    }
};
class CLCLockBlock: public CInterface {
    ...
    unsigned got, lnum;
public:
    CLCLockBlock(ReadWriteLock &_lock, bool readLock, unsigned timeout,
            const char *fname, unsigned _lnum) :
            lock(_lock), lnum(_lnum) {
        got = msTick();
        loop {
            if (readLock) {
            ...
            PROGLOG("CLCLockBlock(write=%d) timeout %s(%d), took %d ms",
                    !readLocked, fname, lnum, got - msTick());
            PrintStackReport();
        }
        got = msTick();
    ...
    ~CLCLockBlock() {
        ...
        if (e > readWriteSlowTracing) {
            ...
            s.append(!readLocked).append(",lnum=").append(lnum).append(
                    ") took ").append(e).append(" ms");
            DBGLOG("%s", s.str());
...
public:
    CLCReadLockBlock(ReadWriteLock &lock, unsigned timeout, const char *fname,
            unsigned lnum) :
            CLCLockBlock(lock, true, timeout, fname, lnum) {
    }
};
class CLCWriteLockBlock: public CLCLockBlock {
public:
    CLCWriteLockBlock(ReadWriteLock &lock, unsigned timeout, const char *fname,
            unsigned lnum) :
            CLCLockBlock(lock, false, timeout, fname, lnum) {
    }
};
#ifdef USECHECKEDCRITICALSECTIONS
...
class CFitArray {
    ...
    inline void _ensure(unsigned _size) {
        if (size < _size) {
            ptrs = (CRemoteTreeBase **) checked_realloc(ptrs,
                    _size * sizeof(CRemoteTreeBase *),
                    size * sizeof(CRemoteTreeBase *), -100);
    ...
    CRemoteTreeBase *_queryElem(__int64 id) {
        ...
        if (!ret)
            return NULL;
        if ((memsize_t) ret >= (memsize_t) &ptrs[0]
                && (memsize_t) ret < (memsize_t) &ptrs[size])
...
public:
    ...
    unsigned maxElements() const {
        return nextId;
    } // actual is nextId - however many in free chain
};
...
enum IncInfo {
    ...
    IncConnect = 0x04,
    IncDisconnect = 0x08,
    IncDisconnectDelete = 0x24
};

StringBuffer &constructStoreName(const char *storeBase, unsigned e,
        StringBuffer &res) {
...
////////////////
static CheckedCriticalSection loadStoreCrit, saveStoreCrit, saveIncCrit,
        nfyTableCrit, extCrit, blockedSaveCrit;
...
public:
    TimingStats() :
            count(0), totalTime(0), maxTime(0), minTime((unsigned long) -1), totalSize(
                    0) {
    }
    ...
    unsigned long queryCount() const {
        return count;
    }
    unsigned long queryMeanTime() const {
        return count ?
                (unsigned long) (((double) totalTime * 0.1) / count + 0.5) : 0;
    }
    unsigned long queryMaxTime() const {
        return maxTime / 10;
    }
    unsigned long queryMinTime() const {
        return count ? minTime / 10 : 0;
    }
    unsigned long queryMeanSize() const {
        return count ? (unsigned long) (((double) totalSize) / count + 0.5) : 0;
    }
private:
    unsigned long count;
...
class TimingBlock {
public:
    TimingBlock(TimingStats & _stats) :
            stats(_stats) {
        start = msTick();
    }
    ~TimingBlock() {
        stats.record(msTick() - start);
    }
protected:
    TimingStats & stats;
private:
...
public:
    TimingSizeBlock(TimingStats & _stats) :
            TimingBlock(_stats), size(0) {
    }
    ~TimingSizeBlock() {
        stats.recordSize(size);
    }
    inline void recordSize(unsigned long _size) {
        size = _size;
    }
private:
    unsigned long size;
...
public:
    ...
    const bool &queryStopped() const {
        return stopped;
    }

    inline TimingStats const & queryXactTimingStats() const {
        return xactTimingStats;
    }
    inline TimingStats const & queryConnectTimingStats() const {
        return connectTimingStats;
    }
    inline TimingStats const & queryCommitTimingStats() const {
        return commitTimingStats;
    }
...
public:
    CConnectionSubscriberContainer(ISubscription *subscriber, SubscriptionId id) :
            CSubscriberContainerBase(subscriber, id) {
    }
    bool notify() {
        MemoryBuffer mb;
        return CSubscriberContainerBase::notify(mb);
    }
};
//////////////
...
public:
    ...
    CServerConnection(ISDSConnectionManager &manager, ConnectionId connectionId,
            const char *xpath, SessionId id, unsigned mode, unsigned timeout,
            IPropertyTree *_parent, ConnInfoFlags _connInfoFlags) :
            CConnectionBase(manager, connectionId, xpath, id, mode, timeout), parent(
                    _parent), connInfoFlags(_connInfoFlags) {
        INIT_NAMEDCOUNT;
        subsid = 0;
        established = false;
    }
...
#define DEBUG_HPCC_11202
            ...
            if (pos == NotFound) {
                StringBuffer msg;
                msg.append("ConnectionId=").appendf("%" I64F "x", connectionId).append(
                        ", xpath=").append(xpath).append(", sessionId=").appendf(
                        "%" I64F "x", sessionId).append(", mode=").append(mode).append(
                        ", timeout=");
                if (INFINITE == timeout)
                    msg.append("INFINITE");
                else
    ...
    MemoryBuffer &getInfo(MemoryBuffer &out) {
        out.append(connectionId).append(xpath).append(sessionId).append(mode).append(
                timeout).append(established);
        return out;
    }
    ...
    void addConnInfoFlag(ConnInfoFlags flag) {
        connInfoFlags = (ConnInfoFlags) (((unsigned) connInfoFlags)
                | ((unsigned) flag));
    }

    void closed(SessionId id) {
        LOG(
                MCwarning, unknownJob, "Connection (%" I64F "x) was leaked by exiting client (%" I64F "x) path=%s", connectionId, id, queryXPath());
        aborted(id);
        subsid = 0;
    }
    ...
    void removeSubscriber(SubscriptionId id) {
        ...
        // do not expect a lot of subscribers per connection - probably ~ 1.
        {
    ...
    bool queryEstablished() {
        return established;
    }
    void setEstablished() {
        established = true;
    }
    IPropertyTree *queryParent() {
        return parent;
    }
    void removeRoot() {
        if (parent)
...
public:
    inline void add(const char *qualifier) {
        qualifiers.append(qualifier);
    }
    inline unsigned count() const {
        return qualifiers.ordinality();
    }
    inline const char *item(unsigned i) const {
        return qualifiers.item(i);
    }
};
class CSubscriberContainer: public CSubscriberContainerBase {
    ...
    unsigned depth;
public:
    CSubscriberContainer(ISubscription *subscriber, SubscriptionId id) :
            CSubscriberContainerBase(subscriber, id) {
        ...
        loop {
            ...
            if (NULL == (startQ = queryNextUnquoted(path, '['))) // escaped '[]' chars??
                    {
                if (strippedXpath.length())
                    strippedXpath.append(path);
                break;
            }
            ...
            loop {
                ...
                if (!endQ)
                    throw MakeSDSException(SDSExcpt_SubscriptionParseError,
                            "Missing closing brace: %s", xpath.get());
                ...
                for (unsigned q2 = 0; q2 < qualifiers->count(); q2++) {
                    ...
                    if (numeric) {
                        ...
                        if (!item.queryParent()) {
                            if (qnum != 1)
                                return false;
                        } else if (((PTree *) item.queryParent())->findChild(
                                &item) != qnum - 1)
                            return false;
...
public:
    ...
    CSubscriberContainerList(const char *_xpath) :
            xpath(_xpath) {
        _xpath = xpath.get();
    }
    const char *queryXPath() const {
    ...
    const char *queryFindString() const {
        return queryXPath();
    }
...
class CSubscriberTable: public ThreadSafeSimpleHashTableOf<CSubscriberContainer,
        SubscriptionId> {
public:
    ~CSubscriberTable() {
        _releaseAll();
    ...
    virtual void onAdd(void *et) {
        ...
        CSubscriberContainerList *list = xpathTable.find(
                subscriber->queryXPath());
        if (!list) {
    ...
    CSubscriberContainerList *getQualifiedList(const char *xpath,
            CPTStack &stack) {
        CriticalBlock b(crit);
        ...
        ForEachItemIn(s, *list) {
            ...
            if (subscriber.qualify(stack, false)) {
                if (!results)
                    results = new CSubscriberContainerList(xpath);
                subscriber.Link();
...
class BoolSetBlock {
public:
    BoolSetBlock(bool &_b, bool state = true) :
            b(_b) {
        o = b;
        b = state;
    }
...
public:
    LinkedStringHTMapping(const char *fp, ET &et) :
            OwningStringHTMapping<ET>(fp, et) {
        this->et.Link();
    }
};
...
void writeDelta(StringBuffer &xml, IFile &iFile, const char *msg = "",
        unsigned retrySecs = 0, unsigned retryAttempts = 10) {
    ...
    loop {
        ...
        try {
            ...
            StringBuffer s(msg);
            LOG(
                    MCoperatorError, unknownJob, e, s.append("writeDelta, failed").str());
        }
        if (!exception.get())
            break;
        if (0 == retrySecs)
...
struct BackupQueueItem {
    ...
    enum flagt {
        f_delta = 0x1, f_addext = 0x2, f_delext = 0x3, f_first = 0x10
    };
    BackupQueueItem() :
            edition((unsigned) -1), flags(0) {
...
class CBackupHandler: public CInterface, implements IThreaded {
    ...
    void writeExt(const char *name, const unsigned length, const void *data,
            unsigned retrySecs = 0, unsigned retryAttempts = 10) {
        Owned<IException> exception;
        unsigned _retryAttempts = retryAttempts;
...
public:
    CExternalFile(const char *_ext, const char *_dataPath,
            CBackupHandler &_backupHandler) :
            ext(_ext), dataPath(_dataPath), backupHandler(_backupHandler) {
    }
    const char *queryExt() {
        return ext;
    }
...
class CLegacyBinaryFileExternal: public CExternalFile,
        implements IExternalHandler {
public:
    IMPLEMENT_IINTERFACE
    ;

    CLegacyBinaryFileExternal(const char *dataPath,
            CBackupHandler &backupHandler) :
            CExternalFile("." EF_LegacyBinaryValue, dataPath, backupHandler) {
    }
    ...
    virtual void readValue(const char *name, MemoryBuffer &mb) {
        ...
        if ((unsigned) -1 == sz) {
            ...
            Owned<IException> e = MakeSDSException(SDSExcpt_MissingExternalFile,
                    "%s", filename.str());
    ...
    virtual void read(const char *name, IPropertyTree &owner, MemoryBuffer &mb,
            bool withValue) {
        StringBuffer filename;
        getFilename(filename, name);
    ...
    virtual void write(const char *name, IPropertyTree &tree) {
        ...
        if (remoteBackupLocation.length()) {
            StringBuffer fname(name);
            backupHandler.addExt(fname.append(queryExt()).str(), length,
                    out.detach());
        }
    }
    virtual void remove(const char *name) {
        CExternalFile::remove(name);
    }
    virtual bool isValid(const char *name) {
        return CExternalFile::isValid(name);
    }
    virtual StringBuffer &getName(StringBuffer &fName, const char *base) {
        return CExternalFile::getName(fName, base);
    }
    virtual StringBuffer &getFilename(StringBuffer &fName, const char *base) {
        return CExternalFile::getFilename(fName, base);
    }
};
...
public:
    ...
    CBinaryFileExternal(const char *dataPath, CBackupHandler &backupHandler) :
            CExternalFile("." EF_BinaryValue, dataPath, backupHandler) {
    }
    virtual void resetAsExternal(IPropertyTree &tree) {
        tree.setProp(NULL, (char *) NULL);
    }
...
public:
    ...
    CXMLFileExternal(const char *dataPath, CBackupHandler &backupHandler) :
            CExternalFile("." EF_XML, dataPath, backupHandler) {
    }
    ...
    virtual void write(const char *name, IPropertyTree &tree) {
        ...
        if (remoteBackupLocation.length()) {
            ...
            backupHandler.addExt(fname.append(queryExt()).str(), l,
                    str.detach());
        }
    }
    virtual void remove(const char *name) {
...
public:
    CBranchChange(CRemoteTreeBase &_tree) :
            tree(&_tree), local(PDS_None), state(PDS_None) {
        INIT_NAMEDCOUNT;
    }

    void noteChange(PDState _local, PDState _state) {
        local = _local;
        state = _state;
    }

    void addChildBranch(CBranchChange &child) {
        children.append(child);
    }

    const void *queryFindParam() const {
        return (const void *) &tree;
    }
...
void buildNotifyData(MemoryBuffer &notifyData, PDState state, CPTStack *stack,
        MemoryBuffer *data) {
    if (stack) {
        notifyData.append('/');
        ...
        if (n > 1) {
            ...
            loop {
                ...
                if (child.queryParent()) {
                    char temp[12];
                    unsigned written = numtostr(temp,
                            parent->findChild(&child) + 1);
...
class CSubscriberNotifier: public CInterface {
    ...
    public:
        CChange(MemoryBuffer &_notifyData) :
                notifyData(_notifyData.length(), _notifyData.toByteArray()) {
        }
        MemoryBuffer notifyData;
    };
public:
    CSubscriberNotifier(CSubscriberNotifierTable &_table,
            CSubscriberContainerBase &_subscriber, MemoryBuffer &notifyData) :
    ...
    void notify() {
        ...
        if (subscriber.isUnsubscribed()) {
            ...
            querySubscriptionManager(SDS_PUBLISHER)->remove(
                    subscriber.queryId());
        }
    }
...
class CConnectionSubscriptionManager: CInterface,
...
enum LockStatus {
    LockFailed, LockHeld, LockTimedOut, LockSucceeded
};

extern bool registerClientProcess(ICommunicator *comm, IGroup *& retcoven,unsigned timeout,DaliClientRole role);
static bool covenInit=true;
static CSDSServerStatus *serverstatus=NULL;

IRemoteConnection *doConnect(ISDSConnectionManager& sdsMgr, const char *xpath, SessionId id, unsigned mode, unsigned timeout)
{
...
bool initRemoteDaliCoven(ISDSConnectionManager& sdsMgr, const char* daliIP, unsigned short daliPort, SessionId id, DaliClientRole role)
{
    ...
    IGroup * covengrp;
    if (!registerClientProcess(comm.get(),covengrp,MP_WAIT_FOREVER,role))
        return true;
...
/*
    IRemoteConnection* conn = doConnect(sdsMgr, "Status/Servers/Server", myProcessSession(), RTM_CREATE_ADD | RTM_LOCK_READ | RTM_DELETE_ON_DISCONNECT, 5*60*1000);
    if (conn) {
        IPropertyTree &root = *conn->queryRoot();
        root.setProp("@name","ESPServer");
        StringBuffer node;
    ...
    SessionId id, unsigned mode, unsigned timeout, DaliClientRole role, const char* xpath)
{
    ...
    if (covenInit)
    {
        covenInit = initRemoteDaliCoven(sdsMgr, daliIP, daliPort, id, role);
    }
    IRemoteConnection *conn = doConnect(sdsMgr, xpath, id, mode, timeout);
    /*CMessageBuffer mb;
    mb.append((int)DAMP_SDSCMD_CONNECT | lazyExtFlag);
...
class CCovenSDSManager: public CSDSManagerBase,
        implements ISDSManagerServer,
        implements ISubscriptionManager,
...
public:
    ...
    CCovenSDSManager(ICoven &_coven, IPropertyTree &config,
            const char *dataPath);
    ~CCovenSDSManager();
    ...
    bool unlock(__int64 treeId, ConnectionId connectionId, bool delayDelete =
            false);
    void unlockAll(__int64 treeId);
    void changeLockMode(CServerConnection &connection, unsigned newMode,
            unsigned timeout);
    void clearSDSLocks();
    void lock(CServerRemoteTree &tree, const char *xpath,
            ConnectionId connectionId, SessionId sessionId, unsigned mode,
            unsigned timeout, IUnlockCallback &callback);
    CLock *queryLock(__int64 id) {
        return lockTable.find(&id);
    }
    CSubscriberTable &querySubscriberTable() {
        return subscribers;
    }
    IExternalHandler *queryExternalHandler(const char *handler) {
        if (!handler)
            return NULL;
        CExternalHandlerMapping *mapping = externalHandlers.find(handler);
        return mapping ? &mapping->query() : NULL;
    }
    void handleNotify(CSubscriberContainerBase *subscriber,
            MemoryBuffer &notifyData);
    void startNotification(IPropertyTree &changeTree, CPTStack &stack,
            CBranchChange &changes); // subscription notification
    ...
    bool queryStopped() {
        return server.queryStopped();
    }

    void handleNodeNotify(notifications n, CServerRemoteTree &tree); // server node notification

    void deleteExternal(__int64 index);
    void serializeExternal(__int64 index, IPropertyTree &owner,
            MemoryBuffer &mb, bool withValue);
    void writeExternal(CServerRemoteTree &tree, bool direct = false,
            __int64 existing = 0);
    inline unsigned queryExternalThreshold() {
        return externalSizeThreshold;
    }
    CServerConnection *queryConnection(ConnectionId id);
    CServerConnection *getConnection(ConnectionId id);
    inline CFitArray &queryAllNodes() {
        return allNodes;
    }
    unsigned __int64 getNextExternal() {
        return nextExternal++;
    }
    CServerConnection *createConnectionInstance(CRemoteTreeBase *root,
            SessionId sessionId, unsigned mode, unsigned timeout,
            const char *xpath, CRemoteTreeBase *&tree,
            ConnectionId connectionId, StringAttr *deltaPath,
            Owned<IPropertyTree> &deltaChange,
            Owned<CBranchChange> &branchChange, unsigned &additions);
    void createConnection(SessionId sessionId, unsigned mode, unsigned timeout,
            const char *xpath, CServerRemoteTree *&tree,
            ConnectionId &connectionId, bool primary,
            Owned<LinkingCriticalBlock> &connectCritBlock);
    void disconnect(ConnectionId connectionId, bool deleteRoot = false,
            Owned<CLCLockBlock> *lockBlock = NULL);
    ...
    CSubscriberContainerList *getSubscribers(const char *xpath,
            CPTStack &stack);
    void getExternalValue(__int64 index, MemoryBuffer &mb);
    IPropertyTree *getXPathsSortLimitMatchTree(const char *baseXPath,
            const char *matchXPath, const char *sortby, bool caseinsensitive,
            bool ascending, unsigned from, unsigned limit);
...
// ISDSConnectionManager
    virtual CRemoteTreeBase *get(CRemoteConnection &connection,
            __int64 serverId);
    virtual void getChildren(CRemoteTreeBase &parent,
            CRemoteConnection &connection, unsigned levels);
    virtual void getChildrenFor(CRTArray &childLessList,
            CRemoteConnection &connection, unsigned levels);
    virtual void ensureLocal(CRemoteConnection &connection,
            CRemoteTreeBase &_parent, IPropertyTree *serverMatchTree,
            IPTIteratorCodes flags = iptiter_null);
    virtual IPropertyTreeIterator *getElements(CRemoteConnection &connection,
            const char *xpath);
    virtual void commit(CRemoteConnection &connection,
            bool *disconnectDeleteRoot);
    virtual void changeMode(CRemoteConnection &connection, unsigned mode,
            unsigned timeout, bool suppressReload);
    virtual IPropertyTree *getXPaths(__int64 serverId, const char *xpath,
            bool getServerIds = false);
    virtual IPropertyTreeIterator *getXPathsSortLimit(const char *baseXPath,
            const char *matchXPath, const char *sortby, bool caseinsensitive,
            bool ascending, unsigned from, unsigned limit);
    virtual void getExternalValueFromServerId(__int64 serverId,
            MemoryBuffer &mb);
    virtual bool unlock(__int64 connectionId, bool closeConn,
            StringBuffer &connectionInfo);

// ISDSManagerServer
    virtual IRemoteConnections *connect(IMultipleConnector *mConnect,
            SessionId id, unsigned timeout);
    virtual IRemoteConnection *connect(const char *xpath, SessionId id,
            unsigned mode, unsigned timeout);
    virtual SubscriptionId subscribe(const char *xpath,
            ISDSSubscription &notify, bool sub = true, bool sendValue = false);
    virtual SubscriptionId subscribeExact(const char *xpath,
            ISDSNodeSubscription &notify, bool sendValue = false);
    virtual void unsubscribe(SubscriptionId id);
    virtual void unsubscribeExact(SubscriptionId id);
    virtual ILockInfoCollection *getLocks(const char *ipPattern,
            const char *xpathPattern);
    ...
    virtual void installNotifyHandler(const char *handlerKey,
            ISDSNotifyHandler *handler);
    ...
    virtual unsigned queryExternalSizeThreshold() const {
        return externalSizeThreshold;
    }
    virtual void setExternalSizeThreshold(unsigned _size) {
        externalSizeThreshold = _size;
    }
    virtual bool queryRestartOnError() const {
        return restartOnError;
    }
    virtual void setRestartOnError(bool _restart) {
        restartOnError = _restart;
    }
    unsigned queryRequestsPending() const {
        return coven.probe(RANK_ALL, MPTAG_DALI_SDS_REQUEST, NULL);
    }
    unsigned queryXactCount() const {
        return server.queryXactTimingStats().queryCount();
    }
    unsigned queryXactMeanTime() const {
        return server.queryXactTimingStats().queryMeanTime();
    }
    unsigned queryXactMaxTime() const {
        return server.queryXactTimingStats().queryMaxTime();
    }
    unsigned queryXactMinTime() const {
        return server.queryXactTimingStats().queryMinTime();
    }
    unsigned queryConnectMeanTime() const {
        return server.queryConnectTimingStats().queryMeanTime();
    }
    unsigned queryConnectMaxTime() const {
        return server.queryConnectTimingStats().queryMaxTime();
    }
    unsigned queryCommitMeanTime() const {
        return server.queryCommitTimingStats().queryMeanTime();
    }
    unsigned queryCommitMaxTime() const {
        return server.queryCommitTimingStats().queryMaxTime();
    }
    unsigned queryCommitMeanSize() const {
        return server.queryCommitTimingStats().queryMeanSize();
    }
    virtual void saveRequest();
    virtual IPropertyTree &queryProperties() const;
    virtual IPropertyTreeIterator *getElementsRaw(const char *xpath,
            INode *remotedali = NULL, unsigned timeout = MP_WAIT_FOREVER);
    virtual void setConfigOpt(const char *opt, const char *value);
    virtual unsigned queryCount(const char *xpath);
    virtual bool updateEnvironment(IPropertyTree *newEnv, bool forceGroupUpdate,
            StringBuffer &response);
    IRemoteConnection *connectRemoteDali(const char *xpath, SessionId id,
            unsigned mode, unsigned timeout, const char* daliIP,
            unsigned short daliPort, DaliClientRole role) {
...
private:
    void validateBackup();
    LockStatus establishLock(CLock &lock, __int64 treeId,
            ConnectionId connectionId, SessionId sessionId, unsigned mode,
            unsigned timeout, IUnlockCallback &lockCallback);
    void _getChildren(CRemoteTreeBase &parent, CServerRemoteTree &serverParent,
            CRemoteConnection &connection, unsigned levels);
    void matchServerTree(CClientRemoteTree *local, IPropertyTree &matchTree,
            bool allTail);
...
void CConnectionSubscriptionManager::add(ISubscription *sub,
        SubscriptionId id) {
    MemoryBuffer mb;
...
CDisableLazyFetchBlock::CDisableLazyFetchBlock(CRemoteConnection &_connection) :
        connection(_connection) {
    lazyFetch = connection.setLazyFetch(false);
}
...
/////////////////
/// CPTStack impl.
bool CPTStack::_fill(IPropertyTree &current, const char *xpath,
        IPropertyTree &tail) {
    ...
    if (!_nextSep) {
        ...
        ForEach (*iter)
        {
            if (&tail == &iter->query()) // Afaics, this should not be possible (so this test/block should really be removed)
                    {
                ERRLOG("_fill() - tail (%s) found at intermediate level: %s",
                        tail.queryName(), head.str());
                append(*LINK((PTree *) &iter->query()));
                append(*LINK((PTree *) &current));
                return true;
...
bool CPTStack::fill(IPropertyTree &root, const char *xpath,
        IPropertyTree &tail) {
    kill();
...
CServerConnection::~CServerConnection() {
    ...
    if (!RTM_MODE(mode, RTM_INTERNAL)) {
        ...
        checkedCritBlock.setown(
                new LinkingCriticalBlock(SDSManager->connDestructCrit, __FILE__,
                        __LINE__));
    }
    ptreePath.kill();
    root.clear();
...
void CServerConnection::aborted(SessionId id) {
    LOG(
            MCdebugInfo(100), unknownJob, "CServerConnection: connection aborted (%" I64F "x) sessId=%" I64F "x",connectionId, id);
...
CRemoteTreeBase::CRemoteTreeBase(const char *name, IPTArrayValue *value,
        ChildMap *children) :
    ...
    public:
        ...
        virtual void onAdd(void *e) // ensure memory of constructed multi value elements are no longer orphaned.
            ...
            CServerRemoteTree &tree =
                    *((CServerRemoteTree *) (IPropertyTree *) e);
        ...
        virtual void onRemove(void *e) {
            ...
            bool dounlock;
            {
                CHECKEDCRITICALBLOCK(suppressedOrphanUnlockCrit,
                        fakeCritTimeout);
...
public:
    ...
    CServerRemoteTree(MemoryBuffer &mb) :
            CRemoteTreeBase(mb) {
        init();
    }
    CServerRemoteTree(const char *name = NULL, IPTArrayValue *value = NULL,
            ChildMap *children = NULL) :
            CRemoteTreeBase(name, value, children) {
        init();
    }
    ...
    ~CServerRemoteTree() {
        ...
        if (index) {
            try {
                SDSManager->deleteExternal(index);
            } catch (IException *e) {
                LOG(
                        MCoperatorWarning, unknownJob, e, StringBuffer("Deleting external reference for ").append(queryName()).str());
                e->Release();
            }
        }
        if (SDSManager->queryStopped())
            return; // don't bother building up free list that will never be used hence (could get v. big/slow)
        if (isSubscribed())
    ...
    virtual bool isEquivalent(IPropertyTree *tree) {
        return (NULL != QUERYINTERFACE(tree, CServerRemoteTree));
    }

    PDState processData(CServerConnection &connection,
            IPropertyTree &changeTree, MemoryBuffer &newIds);
    ...
    virtual bool removeTree(IPropertyTree *_child) {
        if (!_child)
            return false; dbgassertex(QUERYINTERFACE(_child, CServerRemoteTree));
        Linked<CServerRemoteTree> child =
                static_cast<CServerRemoteTree *>(_child);
    ...
    virtual void setServerId(__int64 _serverId) {
        if (serverId && serverId != _serverId)
            WARNLOG(
                    "Unexpected - client server id mismatch in %s, id=%" I64F "x",
                    queryName(), _serverId);
        CRemoteTreeBase::setServerId(_serverId);
    }

    virtual CSubscriberContainerList *getSubscribers(const char *xpath,
            CPTStack &stack) {
        return SDSManager->getSubscribers(xpath, stack);
    }

    IPropertyTree *create(const char *name = NULL, IPTArrayValue *value = NULL,
            ChildMap *children = NULL, bool existing = false) {
    ...
    virtual void createChildMap() {
        children = new COrphanHandler();
    }

    inline bool testExternalCandidate() {
        // maybe be other cases.
        return (value
                && value->queryValueSize()
                        >= SDSManager->queryExternalThreshold());
    }

    void serializeCutOffRT(MemoryBuffer &tgt, int cutoff = -1, int depth = 0,
            bool extValues = true) {
        serializeSelfRT(tgt, extValues);
        serializeCutOffChildrenRT(tgt, cutoff, depth, extValues);
    }

    void serializeCutOffChildrenRT(MemoryBuffer &tgt, int cutoff = -1,
            int depth = 0, bool extValues = true) {
        ...
        if (cutoff < 0 || depth < cutoff) {
            ...
            while (iter->isValid()) {
                ...
                CServerRemoteTree *child = QUERYINTERFACE(_child,
                        CServerRemoteTree);
                assertex(child);
    ...
    virtual bool isCompressed(const char *xpath = NULL) const {
        if (isAttribute(xpath))
            return false;
        if (CRemoteTreeBase::isCompressed(xpath))
            return true;
...
private:
    PDState processData(IPropertyTree &changeTree,
            Owned<CBranchChange> &parentBranchChange, MemoryBuffer &newIds);
...
public:
    CNodeSubscriberContainer(ISubscription *subscriber, SubscriptionId id,
            bool _sendValue, const char *_xpath) :
            CSubscriberContainerBase(subscriber, id), sendValue(_sendValue), xpath(
                    _xpath) {
    }
    bool querySendValue() const {
        return sendValue;
    }
    void add(CServerRemoteTree &node) {
        nodes.append(node);
    }
    ICopyArrayOf<CServerRemoteTree> &queryNodes() {
        return nodes;
    }
...
class CNodeSubscriptionManager: public CSimpleInterface,
        implements INodeSubscriptionManager {
public:
    ...
    public:
        CNodeSubscriberContainerList(CServerRemoteTree *_node) :
                node(_node) {
        }
        const void *queryFindParam() const {
            return &node;
        }
        ICopyArrayOf<CNodeSubscriberContainer> &querySubscribers() {
            return subscribers;
        }
        void getSubscribers(
        ...
        void add(CNodeSubscriberContainer &subscriber) {
        ...
        while (subscribers.ordinality()) {
            ...
            if (subscriber->querySendValue()) {
                if (1 != lastSendValue) // overkill unless many subscribers to same node
                    ...
                    buildNotifyData(sendValueNotifyData.clear(), state, NULL,
                            &mb);
                    lastSendValue = 1;
                }
                SDSManager->handleNotify(subscriber.getClear(),
                        sendValueNotifyData);
            } else {
    ...
    void _notify(CServerRemoteTree *node, PDState state) {
        CNodeSubscriberContainerList *subscriberList =
                subscriberListByNode.find(node);
    ...
    void _removeNode(CServerRemoteTree *node, SubscriptionId id) {
        ...
        assertex(subscriberList);
        ICopyArrayOf<CNodeSubscriberContainer> &subscribers =
                subscriberList->querySubscribers();
        ForEachItemInRev(s, subscribers) {
            ...
            if (0 == id) // remove all associated subscribers (node being deleted)
                    {
                ICopyArrayOf<CServerRemoteTree> &nodes =
                        subscriber.queryNodes();
...
public:
    ...
    CNodeSubscriptionManager(CCovenSDSManager &_owner) :
            owner(_owner) {
    }
    ...
    MemoryBuffer &collectSubscribers(MemoryBuffer &out) const {
        ...
        SuperHashIteratorOf<CNodeSubscriberContainer> sdsNodeIter(
                subscribersById);
        ForEach(sdsNodeIter)
...
void populateWithServerIds(IPropertyTree *matchParent,
        CRemoteTreeBase *parent) {
    ...
    ForEach (*iter)
    {
        ...
        CRemoteTreeBase *child = (CRemoteTreeBase *) parent->queryPropTree(
                path.str());
        assertex(child);
...
PDState CServerRemoteTree::checkChange(IPropertyTree &changeTree,
        CBranchChange &parentBranchChange) {
    ...
    ForEach(*iter)
    {
        ...
        switch (name[0]) {
        case 'R': {
            IPropertyTree *idTree = SDSManager->queryRegisteredTree(
                    e.getPropInt64("@id"));
            if (!idTree)
                throw MakeSDSException(SDSExcpt_OrphanedNode,
                        "renaming %s to %s", e.queryProp("@from"),
                        e.queryProp("@to"));
            ...
            if (renameTree(idTree, e.queryProp("@to"))) {
                ...
                Owned<CBranchChange> childChange = new CBranchChange(
                        *(CRemoteTreeBase *) idTree);
        ...
        case 'D': {
            ...
            if (!removeTree(idTree))
                throw MakeSDSException(-1,
                        "::checkChange - Failed to remove child(%s) from parent(%s) at %s(%d)",
                        idTree->queryName(), queryName(), __FILE__, __LINE__);
            mergePDState(res, PDS_Structure);
            PDState _res = res;
    ...
    if (changeTree.getPropBool("@localValue")) {
        ...
        setValue(
                v ? new CPTValue(v->queryValueRawSize(), v->queryValueRaw(),
                            binary, true, v->isCompressed()) :
                    NULL, binary);
        ...
        if (binary != isBinary(NULL))
            throw MakeSDSException(-1,
                    "Error attempting to append binary and non-binary data together, in node: %s",
                    queryName());
        __int64 index = getPropInt64(EXT_ATTR);
        MemoryBuffer mb;
        if (index) {
...
PDState CServerRemoteTree::processData(CServerConnection &connection,
        IPropertyTree &changeTree, MemoryBuffer &newIds) {
...
PDState CServerRemoteTree::processData(IPropertyTree &changeTree,
        Owned<CBranchChange> &parentBranchChange, MemoryBuffer &newIds) {
    ...
    if (iter->first()) {
        ...
        do {
            ...
            if (childChanges.getPropBool("@new")) {
                child = (CServerRemoteTree *) createChild(
                        childChanges.getPropInt("@pos", -1),
                        childChanges.queryProp("@name"));
                newIds.append(child->queryServerId());
                mergePDState(localChange, PDS_Added);
                mergePDState(res, PDS_Added);
            } else {
                child = (CServerRemoteTree *) SDSManager->queryRegisteredTree(
                        childChanges.getPropInt64("@id"));
#ifdef SIBLING_MOVEMENT_CHECK
...
#endif
                if (NULL == child)
                    throw MakeSDSException(SDSExcpt_ClientCacheDirty,
                            "child(%s)  not found in parent(%s) at %s(%d)",
                            childChanges.queryProp("@name"), queryName(),
                            __FILE__, __LINE__);
                int pos = findChild(child);
                if (NotFound == pos)
                    throw MakeSDSException(SDSExcpt_ClientCacheDirty,
                            "child(%s) not found in parent(%s) at %s(%d)",
                            child->queryName(), queryName(), __FILE__,
                            __LINE__);
            ...
            mergePDState(res,
                    child->processData(childChanges, branchChange, newIds));
        } while (iter->next());
    } else
        branchChange->noteChange(localChange, res);
...
class CLock: public CInterface, implements IInterface {
    ...
    Semaphore sem;
    StringAttr xpath;__int64 treeId;
    bool exclusive;
    ...
    bool validateConnectionSessions() {
        ...
        try {
            ...
            ForEachItemIn(e, entries)
            {
                ...
                Owned<INode> node = querySessionManager().getProcessSessionNode(
                        lD->sessId);
                if (node) {
                    SessionId nodeSessId =
                            querySessionManager().lookupProcessSession(node);
                    if (nodeSessId) {
                        if (lD->sessId != nodeSessId) {
                            StringBuffer out(
                                    "Removing stale connection session [");
                            out.appendf(
                                    "%" I64F "x], connectionId [%" I64F "x]",
                                    lD->sessId,
                                    *((ConnectionId *) imap.getKey()));
                            ...
                            PROGLOG("Validating connection to %s",
                                    nodeStr.str());
                            if (!queryWorldCommunicator().verifyConnection(node,
                                    LOCKSESSCHECK)) {
                                StringBuffer out(
                                        "Terminating connection session to ");
                                out.append(nodeStr);
                                out.append(" [");
                                out.appendf(
                                        "%" I64F "x], connectionId [%" I64F "x]",
    ...
    LockStatus doLock(unsigned mode, unsigned timeout, ConnectionId id,
            SessionId sessionId, IUnlockCallback &callback,
            bool change = false) {
        if (INFINITE == timeout) {
            loop {
                ...
                if (lockStatus == LockSucceeded || lockStatus == LockHeld)
                    ...
                    if (timedout) {
                        if (!sem.wait(0)) {
                            waiting--;
                            StringBuffer s(
                                    "Infinite timeout lock still waiting: ");
                            toString(s);
                            PROGLOG("%s", s.str());
                        }
                        {
                ...
                if (lockStatus == LockSucceeded || lockStatus == LockHeld)
                    ...
                    waiting++;
                    {
                        ...
                        if (tm.timedout(&remaining)
                                || !sem.wait(
                                        remaining > LOCKSESSCHECK ?
                                                LOCKSESSCHECK : remaining))
                            timedout = true;
                        callback.block();
                    }
                    if (timedout) {
                        ...
                        if (tm.timedout()) {
                            if (disconnects) // if some sessions disconnected, one final try
                            {
                                if (!SDSManager->queryConnection(id))
                                    return LockFailed;
                                lockStatus = tryLock(mode, id, sessionId,
                                        change);
                                if (lockStatus == LockSucceeded
                                        || lockStatus == LockHeld)
                                    return lockStatus;
                            }
                            break;
    ...
    LockStatus tryLock(unsigned mode, ConnectionId id, SessionId sessId,
        ...
        if (changingMode) {
            ...
            if (existingLd) {
                if ((existingLd->mode & RTM_LOCKBASIC_MASK)
                        == (mode & RTM_LOCKBASIC_MASK))
...
public:
    ...
    CLock(CLockTable &_table, __int64 _treeId, IdPath &_idPath,
            const char *_xpath, unsigned mode, ConnectionId id,
            SessionId sessId) :
            table(_table), treeId(_treeId), xpath(_xpath), exclusive(false), sub(
                    0), readLocks(0), holdLocks(0), waiting(0), pending(0) {
        INIT_NAMEDCOUNT;
    ...
    LockStatus lock(unsigned mode, unsigned timeout, ConnectionId id,
            SessionId sessionId, IUnlockCallback &callback) {
        bool ret = false;
        CPendingLockBlock b(*this); // carefully placed, removePending can destroy this, therefore must be destroyed last
        {
    ...
    void changeMode(ConnectionId id, SessionId sessionId, unsigned newMode,
            unsigned timeout, IUnlockCallback &callback) {
        CPendingLockBlock b(*this); // carefully placed, removePending can destroy this.
        CHECKEDCRITICALBLOCK(crit, fakeCritTimeout);
        LockStatus result = doLock(newMode, timeout, id, sessionId, callback,
                true);
        if (result != LockSucceeded) {
            StringBuffer s;
            switch (result) {
            case LockFailed:
                throw MakeSDSException(SDSExcpt_ConnectionAbsent,
                        "Lost connection performing changeMode on connection to : %s",
                        xpath.get());
            case LockTimedOut:
                throw MakeSDSException(SDSExcpt_LockTimeout,
                        "Lock timeout performing changeMode on connection to : %s, existing lock info: %s",
                        xpath.get(), toString(s).str());
            case LockHeld:
                throw MakeSDSException(SDSExcpt_LockHeld,
                        "Lock is held performing changeMode on connection to : %s, existing lock info: %s",
                        xpath.get(), toString(s).str());
            }
        }
    }
...
CPendingLockBlock::CPendingLockBlock(CLock &_lock) :
        lock(_lock) {
    lock.addPending();
}
...
CSDSTransactionServer::CSDSTransactionServer(CCovenSDSManager &_manager) :
        Thread("SDS Manager, CSDSTransactionServer"), manager(_manager), CTransactionLogTracker(
                DAMP_SDSCMD_MAX) {
    stopped = true;
}
...
int CSDSTransactionServer::run() {
    ...
    CMessageHandler<CSDSTransactionServer> handler("CSDSTransactionServer",
            this, &CSDSTransactionServer::processMessage, &manager, 100,
            TIMEOUT_ON_CLOSEDOWN);
    stopped = false;
    CMessageBuffer mb;
    while (!stopped) {
        try {
#ifdef TRACE_QWAITING
            unsigned waiting = coven.probe(RANK_ALL, MPTAG_DALI_SDS_REQUEST,
                    NULL);
            ...
            if ((waiting > lastwaiting + 25) || (waiting < lastwaiting / 2)
                ...
                DBGLOG("QPROBE: MPTAG_DALI_SDS_REQUEST has %d waiting",
                        waiting);
                lastwaiting = waiting;
                lasttick = msTick();
            }
            ...
            if (coven.recv(mb, RANK_ALL, MPTAG_DALI_SDS_REQUEST, NULL)) {
                ...
                try {
                    SdsCommand action;
                    mb.read((int &) action);
                    action = (SdsCommand) (((unsigned) action)
                            & ~DAMP_SDSCMD_LAZYEXT);
                    switch (action) {
                    case DAMP_SDSCMD_CONNECT:
                    case DAMP_SDSCMD_MCONNECT:
                    ...
                    case DAMP_SDSCMD_VERSION: {
                        ...
                        throw MakeStringException(-1,
                                "Client too old to communicate with this dali");
                    }
#endif
                    case DAMP_SDSCMD_DIAGNOSTIC: {
                        TimingBlock xactTimingBlock(xactTimingStats);
                        ...
                        case DIAG_CMD_LOCKINFO: {
                            ...
                            Owned<ILockInfoCollection> lockInfoCollection =
                                    SDSManager->getLocks(ipPattern,
                                            xpathPattern);
                    ...
                    case DAMP_SDSCMD_UPDTENV: {
                        ...
                        StringBuffer response;
                        bool result = manager.updateEnvironment(newEnv,
                                forceGroupUpdate, response);
                        mb.clear().append(DAMP_SDSREPLY_OK).append(result).append(
                                response);
                        break;
                    }
                    default:
                        throw MakeSDSException(SDSExcpt_UnrecognisedCommand,
                                "%d", action);
                    }
                } catch (IException *e) {
                    mb.clear();
                ...
                if (mb.length()) {
                    try {
                        ...
                        LOG(
                                MCwarning, unknownJob, e, "Failed to reply to client (CSDSTransactionServer thread)");
                        e->Release();
...
// backward compat.
bool checkOldFormat(CServerRemoteTree *parentServerTree, IPropertyTree *tree,
        MemoryBuffer &mb) {
    CPState state;
    mb.read((int &) state);
    ...
    if (state) {
        if (CPS_Renames & state) {
            loop {
                ...
                if (child) {
                    ...
                    if (NotFound == pos)
                        throw MakeSDSException(SDSExcpt_ClientCacheDirty,
                                "::checkChange - child(%s) not found in parent(%s) at %s(%d)",
                                child->queryName(),
                                parentServerTree->queryName(), __FILE__,
                                __LINE__);
        ...
        if (CPS_AttrDeletions & state) {
            ...
            if (count) {
                ...
                IPropertyTree *t = tree->addPropTree(ATTRDELETE_TAG,
                        createPTree());
                for (c = 0; c < count; c++) {
                    StringAttr attr;
                    mb.read(attr);
        ...
        if (CPS_Changed & state) {
            ...
            if (clientTree->queryValue()) {
                ...
                ((PTree *) tree)->setValue(
                        new CPTValue(0, NULL, false, true, false), false);

            Owned<IAttributeIterator> attrs = clientTree->getAttributes();
            IPropertyTree *t = createPTree();
            if (attrs->first()) {
                do {
                    t->setProp(attrs->queryName(),
                            clientTree->queryProp(attrs->queryName()));
                } while (attrs->next());
...
bool translateOldFormat(CServerRemoteTree *parentServerTree,
        IPropertyTree *parentTree, MemoryBuffer &mb) {
    ...
    if (hasChildren) {
        loop {
            ...
            if (0 == id) {
                ...
                assertex(parentServerTree);
                serverTree =
                        (CServerRemoteTree *) SDSManager->queryRegisteredTree(
                                id);
                assertex(serverTree);
                ...
                if (NotFound == pos)
                    throw MakeSDSException(SDSExcpt_ClientCacheDirty,
                            "child(%s) not found in parent(%s) at %s(%d)",
                            serverTree->queryName(),
                            parentServerTree->queryName(), __FILE__, __LINE__);
...
void CSDSTransactionServer::processMessage(CMessageBuffer &mb) {
    ...
    try {
        ...
        case DAMP_SDSCMD_CONNECT: {
            ...
            if (queryTransactionLogging())
                transactionLog.log("xpath='%s' mode=%d", xpath.get(),
                        (unsigned) mode);
            Owned<LinkingCriticalBlock> connectCritBlock =
                    new LinkingCriticalBlock(manager.connectCrit, __FILE__,
                            __LINE__);
            if (RTM_CREATE == (mode & RTM_CREATE_MASK)
                    || RTM_CREATE_QUERY == (mode & RTM_CREATE_MASK))
                lockBlock.setown(
                        new CLCWriteLockBlock(manager.dataRWLock,
                                readWriteTimeout, __FILE__, __LINE__));
            else
                lockBlock.setown(
                        new CLCReadLockBlock(manager.dataRWLock,
                                readWriteTimeout, __FILE__, __LINE__));
            if (queryTransactionLogging())
            ...
            manager.createConnection(id, mode, timeout, xpath, _tree,
                    connectionId, true, connectCritBlock);
            if (connectionId)
                tree.setown(_tree);
            ...
            if (connectionId) {
                if (0 == id) {
                    StringBuffer str(
                            "Dali client passing sessionid=0 to connect (xpath=");
                    str.append(xpath).append(", mode=").append(mode).append(
                            ", connectionId=").appendf("%" I64F "x",
                            connectionId).append(")");
                    WARNLOG("%s", str.str());
                }
                mb.clear();
                ...
                tree->serializeCutOffRT(mb,
                        RTM_SUB & mode ? FETCH_ENTIRE :
                        tree->getPropBool("@fetchEntire") ?
                                FETCH_ENTIRE_COND : 0, 0, getExt);
            } else {
                mb.clear();
        ...
        case DAMP_SDSCMD_MCONNECT: {
            ...
            Owned<IMultipleConnector> mConnect = deserializeIMultipleConnector(
                    mb);
            mb.clear();

            lockBlock.setown(
                    new CLCReadLockBlock(manager.dataRWLock, readWriteTimeout,
                            __FILE__, __LINE__));

            try {
                Owned<CRemoteConnections> remoteConnections =
                        new CRemoteConnections;
                unsigned c;
                for (c = 0; c < mConnect->queryConnections(); c++) {
                    ...
                    if (queryTransactionLogging())
                        transactionLog.extra(", xpath='%s', mode=%d",
                                xpath.get(), mode);
                    connectionId = 0;
                    CServerRemoteTree *_tree;
                    Owned<CServerRemoteTree> tree;
                    Owned<LinkingCriticalBlock> connectCritBlock =
                            new LinkingCriticalBlock(manager.connectCrit,
                                    __FILE__, __LINE__);
                    manager.createConnection(id, mode, timeout, xpath, _tree,
                            connectionId, true, connectCritBlock);
                    if (connectionId)
                    ...
                    if (connectionId) {
                        if (0 == id) {
                            StringBuffer str(
                                    "Dali client passing sessionid=0 to multi connect (xpath=");
                            str.append(xpath).append(", mode=").append(mode).append(
                                    ", connectionId=").appendf("%" I64F "x",
                                    connectionId).append(")");
                            WARNLOG("%s", str.str());
                        }
                        CRemoteConnection *conn = new CRemoteConnection(
                                *SDSManager, connectionId, xpath, id, mode,
                                timeout);
                        assertex(conn);
            ...
            {
                StringBuffer s(
                        "(Unknown exception); Failed to establish locks to multiple paths: ");
                getMConnectString(mConnect, s);
                throw;
            }
            break;
        ...
        case DAMP_SDSCMD_GET2: {
            ...
            if (queryTransactionLogging()) {
                ...
                transactionLog.extra(", xpath='%s', node=%s",
                        conn ? conn->queryXPath() : "???",
                        tree ? tree->queryName() : "???");
            }
            mb.clear();
            if (!tree) {
                if (DAMP_SDSCMD_GET2 == action)
                    ...
                    CServerConnection *connection = manager.queryConnection(
                            connectionId);
                    StringBuffer s;
                    if (connection) {
                    ...
                    throw MakeSDSException(SDSExcpt_UnknownTreeId,
                            "get: treeId = (%d), connection={ %s }",
                            (unsigned) serverId, s.str());
                }
            } else {
                mb.append((int) DAMP_SDSREPLY_OK);
                tree->serializeCutOffRT(mb,
                        tree->getPropBool("@fetchEntire") ? -1 : 0, 0, getExt);
            }
            break;
        }
        case DAMP_SDSCMD_GETCHILDREN:
        case DAMP_SDSCMD_GETCHILDREN2: {
            ...
            loop {
                ...
                unsigned levels;
                mb.read(levels);
                Owned<CServerRemoteTree> parent = manager.getRegisteredTree(
                        serverId);
                if (!parent) {
                    if (DAMP_SDSCMD_GETCHILDREN2 == action)
                        ...
                        if (first) // if only one, can acheive without serialization change.
                        {
                            ...
                            CServerConnection *connection =
                                    manager.queryConnection(connectionId);
                            StringBuffer s;
                            if (connection) {
                                s.append("path=").append(
                                        connection->queryXPath());
                                s.append(", mode=").append(
                                        connection->queryMode());
                            } else
                                s.append("Missing connection!");
                            throw MakeSDSException(SDSExcpt_UnknownTreeId,
                                    "GETCHILDREN: Failed to locate parent (%d), connection={ %s }",
                                    (unsigned) serverId, s.str());
                        }
                    }
                } else {
                    if (DAMP_SDSCMD_GETCHILDREN2 == action)
                        replyMb.append(true);
                    parent->serializeCutOffChildrenRT(replyMb,
                            0 == levels ? (unsigned) -1 : levels, 0, getExt);
                    if (queryTransactionLogging())
        ...
        case DAMP_SDSCMD_GETELEMENTS: {
            ...
            Owned<IPropertyTreeIterator> iter =
                    connection->queryRoot()->getElements(xpath);
            ICopyArrayOf<CServerRemoteTree> arr;
            ForEach (*iter)
                arr.append((CServerRemoteTree &) iter->query());
            CMessageBuffer replyMb;
            replyMb.init(mb.getSender(), mb.getTag(), mb.getReplyTag());
            replyMb.append((int) DAMP_SDSREPLY_OK);
        ...
        case DAMP_SDSCMD_DATA: {
            ...
            if (queryTransactionLogging()) {
                CServerConnection *conn = manager.queryConnection(connectionId);
                transactionLog.log("disconnect=%s, data=%s, deleteRoot=%s",
                        disconnect ? "true" : "false", data ? "true" : "false",
                        deleteRoot ? "true" : "false");
            }
            Owned<CLCLockBlock> lockBlock;
            {
            ...
            try {
                if (queryTransactionLogging())
                    transactionLog.extra(", xpath='%s'",
                            connection->queryXPath());

                CServerRemoteTree *tree =
                        data ? (CServerRemoteTree *) connection->queryRoot() : (CServerRemoteTree *) connection->queryRootUnvalidated();
                MemoryBuffer newIds;
                ...
                if (changeTree
                        && tree->processData(*connection, *changeTree,
                                newIds)) { // something commited, if RTM_Create was used need to remember this.
                ...
                if (block0.slow()) {
                    block0.appendMsg(", xpath=").append(
                            connection->queryXPath());
                    block0.appendMsg(", block size = ").append(mb.length());
                }
            } catch (IException *) {
                if (disconnect)
                    manager.disconnect(connectionId, deleteRoot,
                            (data || deleteRoot) ? NULL : &lockBlock);
                throw;
            }
            if (disconnect)
                manager.disconnect(connectionId, deleteRoot,
        ...
        case DAMP_SDSCMD_CHANGEMODE: {
            ...
            Linked<CServerConnection> connection = manager.queryConnection(
                    connectionId);
            if (!connection)
                throw MakeSDSException(SDSExcpt_ConnectionAbsent,
                        " [changeMode]");
            CServerRemoteTree *tree =
                    (CServerRemoteTree *) connection->queryRoot();
            assertex(tree);
            if (queryTransactionLogging())
            ...
            if (!manager.queryConnection(connectionId)) {
                ...
                throw MakeSDSException(SDSExcpt_AbortDuringConnection,
                        " during changeMode");
            }
            mb.append((int) DAMP_SDSREPLY_OK);
        ...
        case DAMP_SDSCMD_GETXPATHSPLUSIDS: {
            ...
            mb.clear();
            CHECKEDDALIREADLOCKBLOCK(manager.dataRWLock, readWriteTimeout);
            Owned<IPropertyTree> matchTree = SDSManager->getXPaths(serverId,
                    xpath, DAMP_SDSCMD_GETXPATHSPLUSIDS == action);
            if (matchTree) {
                mb.append((int) DAMP_SDSREPLY_OK);
        ...
        case DAMP_SDSCMD_GETXPATHSCRITERIA: {
            ...
            if (queryTransactionLogging()) {
                transactionLog.log(
                        "xpath='%s',matchXPath='%s',sortBy='%s',acscending=%s,from=%d,limit=%d",
                        xpath.get(), matchXPath.get(), sortBy.get(),
                        ascending ? "true" : "false", from, limit);
            }
            mb.clear();
            CHECKEDDALIREADLOCKBLOCK(manager.dataRWLock, readWriteTimeout);
            Owned<IPropertyTree> matchTree =
                    SDSManager->getXPathsSortLimitMatchTree(xpath, matchXPath,
                            sortBy, caseinsensitive, ascending, from, limit);
            if (matchTree) {
                mb.append((int) DAMP_SDSREPLY_OK);
                matchTree->serialize(mb);
            } else
        ...
        case DAMP_SDSCMD_GETEXTVALUE: {
            ...
            if (queryTransactionLogging()) {
                Owned<CServerRemoteTree> idTree =
                        (CServerRemoteTree *) SDSManager->getRegisteredTree(
                                serverId);
        ...
        case DAMP_SDSCMD_GETELEMENTSRAW: {
            ...
            if ('/' == *xpath)
                ++xpath;
            Owned<IPropertyTreeIterator> iter =
                    manager.queryRoot()->getElements(xpath);
            ForEach (*iter)
            {
                ++count;
                IPropertyTree &e = iter->query();
    ...
    {
        Owned<IException> e = MakeSDSException(-1,
                "Unknown server exception processing client action: %d",
                action);
        mb.clear();
        mb.append((int) DAMP_SDSREPLY_ERROR);
        StringBuffer s;
    ...
    try {
        ...
        LOG(
                MCwarning, unknownJob, e, "Failed to reply to client (processMessage)");
        e->Release();
    } catch (IException *e) {
        ...
        try {
            coven.reply(mb);
            LOG(
                    MCdebugInfo(100), unknownJob, "Failed to reply, but succeeded sending initial reply error to client");
        }
        catch (IException *e)
        {
...
public:
    ...
    virtual bool matchesFindParam(const void *et, const void *fp,
            unsigned) const {
        return *(ConnectionId *) et == *(ConnectionId *) fp;
    }
};

static bool retryRename(const char *from, const char *to, unsigned maxAttempts,
        unsigned delay) {
    ...
    loop {
        ...
        try {
            ...
            EXCLOG(
                    e, errTxt.append(from).append(" to ").append(to).append(", retrying...").str());
            e->Release();
        }
...
IPropertyTree *loadStore(const char *storeFilename, IPTreeMaker *iMaker,
        unsigned crcValidation, bool logErrorsOnly = false, const bool *abort =
                NULL) {
    ...
    try {
        ...
        if (!iFileIOStore)
            throw MakeSDSException(SDSExcpt_OpenStoreFailed, "%s",
                    storeFilename);
        ...
        root.setown(
                (CServerRemoteTree *) createPTree(*ios, ipt_none,
                        ptr_ignoreWhiteSpace, iMaker));
        ios.clear();
        unsigned crc = crcPipeStream->queryCrc();

        if (crcValidation && crc != crcValidation)
            LOG(
                    MCoperatorWarning, unknownJob, "Error processing store %s - CRC ERROR (file size=%" I64F "d, validation crc=%x, calculated crc=%x)", storeFilename, iFileIOStore->size(), crcValidation, crc); // not fatal yet (maybe later)
        }
        catch (IException *e)
        {
            if (!abort)
...
class CLightCoalesceThread: public CInterface, implements ICoalesce {
    ...
    public:
        CThreaded() :
                Thread("CLightCoalesceThread") {
            coalesce = NULL;
        }
        void init(CLightCoalesceThread *_coalesce) {
            coalesce = _coalesce;
            start();
        }
        virtual int run() {
...
public:
    IMPLEMENT_IINTERFACE
    ;
    CLightCoalesceThread(IPropertyTree &_config, IStoreHelper *_iStoreHelper) :
            config(&_config), iStoreHelper(_iStoreHelper) {
        stopped = false;
        idlePeriod = config->getPropInt("@lCIdlePeriod", DEFAULT_LCIDLE_PERIOD)
                * 1000;
        minimumTimeBetweenSaves = config->getPropInt("@lCMinTime",
                DEFAULT_LCMIN_TIME) * 1000;
        ...
        if (quietStartTimeStr && !quietEndTimeStr) {
            WARNLOG(
                    "Start time for quiet period specified without end time, ignoring times");
            quietStartTime.clear();
        } else if (quietEndTimeStr && *quietEndTimeStr) {
            if (!quietStartTimeStr)
                WARNLOG(
                        "End time for quiet period specified without start time, ignoring times");
            else {
            ...
            if (!sem.wait(idlePeriod)) {
                if (writeTransactionsNow != lastSaveWriteTransactions) {
                    if (quietStartTime) {
                        ...
                        if (within24) {
                            if (!(nowTime->compare(*quietStartTime) >= 0
                                    && nowTime->compare(*quietEndTime) <= 0))
                                continue; // if outside quiet period within 0-24
                        } else if (nowTime->compare(*quietEndTime) > 0
                                && nowTime->compare(*quietStartTime) < 0)
                    ...
                    if (lastEdition == iStoreHelper->queryCurrentEdition()) // if not then something else has saved (e.g. probably sasha)
                            {
                        unsigned transactions = SDSManager->writeTransactions
                                - writeTransactionsNow; // don't care about rollover.
                        if (0 == transactions
                                || (0 != idleRate && idlePeriod >= 60000
                                        && (transactions / (idlePeriod / 60000))
                                                <= idleRate)) {
                            StringBuffer filename;
                            ...
                            try {
                                if (iFile->getTime(&createTime, NULL, NULL))
                                    diff = ((int) nowTime.getSimple()
                                            - (int) createTime.getSimple())
                                            * 1000;
                            } catch (IException *e) {
                                StringBuffer errMsg(
                                        "failed to get createtime for : ");
                            ...
                            int period;
                            if (diff <= 0
                                    || diff > (int) minimumTimeBetweenSaves) // <0 - createTime>nowTime, assume time skew and allow save.
                                            {
                                period = minimumTimeBetweenSaves - idlePeriod;
                                if (0 > period)
                                    period = 0;
                                {
                                    CHECKEDCRITICALBLOCK(saveStoreCrit,
                                            fakeCritTimeout);
                                    SDSManager->blockingSave(
                                            &lastSaveWriteTransactions);
                                    lastEdition =
                                            iStoreHelper->queryCurrentEdition();
                                }
                                t = lastWarning = 0;
                            } else
                            ...
                            t += idlePeriod / 1000;
                            if (t / 3600 >= STORENOTSAVE_WARNING_PERIOD
                                    && ((t - lastWarning) / 3600
                                            > (STORENOTSAVE_WARNING_PERIOD / 2))) {
                                WARNLOG("Store has not been saved for %d hours",
                                        t / 3600);
                                lastWarning = t;
                            }
                        }
...
class CUnlockCallback: implements IUnlockCallback { // NB: unblock() always called 1st, then block()
    StringAttr xpath;
    ConnectionId connectionId;
    CServerRemoteTree &tree;
    bool lockedForWrite, unlocked;
public:
    CUnlockCallback(const char *_xpath, ConnectionId _connectionId,
            CServerRemoteTree &_tree) :
            xpath(_xpath), connectionId(_connectionId), tree(_tree), lockedForWrite(
    ...
    void block() {
        ...
        if (e > readWriteSlowTracing) {
            ...
            s.append(lockedForWrite).append(",xpath=").append(xpath).append(
                    ", connectionId=").appendf("%" I64F "x", connectionId).append(
                    ") took ").append(e);
            DBGLOG("%s", s.str());
            if (readWriteStackTracing)
                PrintStackReport();
        ...
        if (tree.isOrphaned())
            throw MakeSDSException(SDSExcpt_OrphanedNode,
                    "Whilst completing lock to %s", xpath.get());
    }
    void unblock() {
        unlocked = true;
...
class CStoreHelper: public CInterface, implements IStoreHelper {
    ...
    void clearStoreInfo(const char *base, const char *location,
            unsigned edition, CStoreInfo *storeInfo = NULL) {
        StringBuffer wcard;
        ...
        if (!storeInfo || !storeInfo->cache) {
            Owned<IDirectoryIterator> dIter = createDirectoryIterator(location,
                    wcard.str());
            ForEach (*dIter)
            {
                loop {
    ...
    void writeStoreInfo(const char *base, const char *location,
            unsigned edition, unsigned *crc, CStoreInfo *storeInfo = NULL) {
        StringBuffer path, filename;
        filename.append(base).append('.').append(edition);
        if (location)
            path.append(location);
    ...
    void updateStoreInfo(const char *base, const char *location,
            unsigned edition, unsigned *crc, CStoreInfo *storeInfo = NULL) {
    ...
    void refreshInfo(CStoreInfo &info, const char *base) {
        ...
        if (info.cache.length()) // avoid directory lookup if poss.
        {
            ...
            if (iFile->exists()) {
                found.set(iFile);
                try {
                    iFileIO.setown(found->open(IFOread));
                } catch (IException *e) {
                    e->Release();
                }
            }
        ...
        if (!iFileIO) {
            ...
            loop {
                ...
                if (totalDelays >= MAXDELAYS)
                    throw MakeSDSException(SDSExcpt_StoreInfoMissing,
                            "store.<edition> file appears to be missing");
                if (abort && *abort)
                    return;
                MilliSleep(delay);
            }
    ...
    void refreshStoreInfo() {
        refreshInfo(storeInfo, "store");
    }
    void refreshDeltaInfo() {
        refreshInfo(deltaInfo, "store");
    }
    ...
    void checkInfo(const char *base, CStoreInfo &info) {
        ...
        if (!dIter->first())
            ...
            throw MakeStringException(0,
                    "Multiple store.X files - only one corresponding to latest dalisds<X>.xml should exist");
    }

    void renameDelta(unsigned oldEdition, unsigned newEdition,
            const char *path) {
    ...
    struct CheckDeltaBlock {
        CheckDeltaBlock(CStoreHelper &_storeHelper) :
                storeHelper(_storeHelper) {
            bool created = false;
            StringBuffer deltaIPStr(storeHelper.location);
            OwnedIFile deltaIPIFile = createIFile(
                    deltaIPStr.append(DELTAINPROGRESS).str());
            activeDetachIPStr.append(DETACHINPROGRESS);
            inactiveDetachIPStr.append(storeHelper.location).append('_').append(
                    DETACHINPROGRESS);
            detachIPIFile.setown(createIFile(inactiveDetachIPStr.str()));

            OwnedIFileIO detachIPIO = detachIPIFile->open(IFOcreate);
            detachIPIO->write(0, sizeof(storeHelper.mySessId),
                    &storeHelper.mySessId);
            detachIPIO.clear();
...
IStoreHelper *createStoreHelper(const char *storeName, const char *location,
        const char *remoteBackupLocation, unsigned configFlags,
        unsigned keepStores, unsigned delay, const bool *abort) {
    if (!storeName)
        storeName = "dalisds";
    return new CStoreHelper(storeName, location, remoteBackupLocation,
            configFlags, keepStores, delay, abort);
}
...
CCovenSDSManager::CCovenSDSManager(ICoven &_coven, IPropertyTree &_config,
    ...
    allNodes.ensure(
            initNodeTableSize ? initNodeTableSize : INIT_NODETABLE_SIZE);
    externalSizeThreshold = config.getPropInt("@externalSizeThreshold",
            DEFAULT_EXTERNAL_SIZE_THRESHOLD);
    ...
    registerSubscriptionManager(SDSCONN_PUBLISHER,
            connectionSubscriptionManager.get());
    registerSubscriptionManager(SDSNODE_PUBLISHER, nodeSubscriptionManager);

    // add external handlers
    Owned<CXMLFileExternal> xmlExternalHandler = new CXMLFileExternal(dataPath,
            backupHandler);
    externalHandlers.replace(
            *new CExternalHandlerMapping(EF_XML, *xmlExternalHandler));
    Owned<CLegacyBinaryFileExternal> legacyBinaryExternalHandler =
            new CLegacyBinaryFileExternal(dataPath, backupHandler);
    externalHandlers.replace(
            *new CExternalHandlerMapping(EF_LegacyBinaryValue,
                    *legacyBinaryExternalHandler));
    Owned<CBinaryFileExternal> binaryExternalHandler = new CBinaryFileExternal(
            dataPath, backupHandler);
    externalHandlers.replace(
            *new CExternalHandlerMapping(EF_BinaryValue,
                    *binaryExternalHandler));

    properties.setown(createPTree("Properties"));
    IPropertyTree *clientProps = properties->setPropTree("Client",
            config.hasProp("Client") ?
                    config.getPropTree("Client") : createPTree());
    ...
    IPropertyTree *throttle = clientProps->setPropTree("Throttle",
            createPTree());
    ...
    if (dataPath.length())
        ...
        if (!GetCurrentDirectory(1024, cwd)) {
            ERRLOG(
                    "CCovenSDSManager: Current directory path too big, setting local path to null");
            cwd[0] = 0;
        }
        rfn.setLocalPath(cwd);
    }
    unsigned keepLastN = config.getPropInt("@keepStores",
            DEFAULT_KEEP_LASTN_STORES);
    StringBuffer path;
    ...
    if (remoteBackupLocation.length()) {
        ...
        backupHandler.init(remoteBackupLocation,
                config.getPropBool("@asyncBackup", true));
    }
    ...
    unsigned configFlags =
            config.getPropBool("@recoverFromIncErrors", true) ?
                    SH_RecoverFromIncErrors : 0;
    configFlags |=
            config.getPropBool("@backupErrorFiles", true) ?
                    SH_BackupErrorFiles : 0;
    iStoreHelper = createStoreHelper(storeName, dataPath, remoteBackupLocation,
            configFlags, keepLastN, 100, &server.queryStopped());
...
void CCovenSDSManager::validateBackup() {
    ...
    if (!compareFiles(iFileDeltaBackup, iFileDelta, false))
        WARNLOG("Delta file backup doesn't exist or differs, filename=%s",
                deltaFilename.str());
    ...
    if (!compareFiles(iFileBackupStore, iFileStore))
        WARNLOG("Store backup file doesn't exist or differs, filename=%s",
                storeFilename.str());
}

static int uint64compare(unsigned __int64 const *i1,
        unsigned __int64 const *i2) {
    if (*i1 == *i2)
        return 0;
    if (*i1 < *i2)
        return -1;
...
public:
    CLegacyFmtItem(const char *_name, const char *_ext, unsigned __int64 _num) :
            name(_name), ext(_ext), num(_num) {
    }
    StringAttr name, ext;
    unsigned __int64 num;
};

static int extNcompareFunc(CInterface * const *_itm1,
        CInterface * const *_itm2) {
    CLegacyFmtItem *itm1 = (CLegacyFmtItem *) *_itm1;
    CLegacyFmtItem *itm2 = (CLegacyFmtItem *) *_itm2;
    if (itm1->num == itm2->num)
        return 0;
    if (itm1->num < itm2->num)
        return -1;
    return 1;
...
void CCovenSDSManager::loadStore(const char *storeName, const bool *abort) {
    ...
    public:
        ...
        virtual IPropertyTree *create(const char *tag) {
            return createServerTree(tag);
        }
    } nodeCreator;
    class CSDSTreeMaker: public CPTreeMaker {
    public:
        CSDSTreeMaker(IPTreeNodeCreator *nodeCreator) :
                CPTreeMaker(ipt_none, nodeCreator) {
        }
        virtual void endNode(const char *tag, unsigned length,
                const void *value, bool binary, offset_t endOffset) {
    ...
    try {
        ...
        LOG(
                MCdebugInfo(100), unknownJob, "loading store %d, storedCrc=%x", iStoreHelper->queryCurrentEdition(), crc);
        root = (CServerRemoteTree *) ::loadStore(storeFilename.str(),
                &treeMaker, crc, false, abort);
        if (!root) {
            StringBuffer s(storeName);
            LOG(
                    MCdebugInfo(100), unknownJob, "Store %d does not exist, creating new store", iStoreHelper->queryCurrentEdition());
            root = new CServerRemoteTree("SDS");
        }
        bool errors;
        Owned<IException> deltaE;
        try {
            iStoreHelper->loadDeltas(root, &errors);
        } catch (IException *e) {
            deltaE.setown(e);
            errors = true;
        }
        ...
        if (environment && *environment) {
            LOG(
                    MCdebugInfo(100), unknownJob, "loading external Environment from: %s", environment);
...
// Copy over any if reference and present either only in primary or backup
        ...
        loop {
            ...
            unsigned __int64 refN =
                    refExts.ordinality() ?
                            refExts.item(0) : (unsigned __int64) -1;
            unsigned __int64 fileN =
                    fileExts.ordinality() ?
                            fileExts.item(0) : (unsigned __int64) -1;
            unsigned fileP = 0;
            unsigned refP = 0;
            unsigned missP = 0;
            while (fileN != ((unsigned __int64) -1)
                    || refN != ((unsigned __int64) -1)) {
                while (fileN == refN) {
                    if (!primary) {
                        bool found = false;
                        while (missP < missingPrimarys.ordinality()) {
                            unsigned __int64 missN = missingPrimarys.item(
                                    missP);
                            if (refN == missN) {
                                ++missP;
                                found = true;
                        ...
                        if (found) {
                            ...
                            PROGLOG(
                                    "Copying missing primary external from backup: %s",
                                    fname.str());
...
// check any marked with legacy formats
        if (legacyFmts.ordinality() && refExts.ordinality()) {
            ...
            ForEachItemIn(l, legacyFmts)
            {
                ...
                if (done)
                    break;
                if (refN == itm.num) {
                    IExternalHandler *extHandler = queryExternalHandler(
                            itm.ext);
                    if (!extHandler) {
                        WARNLOG(
                                "Unknown external extension, external=%s, extension=%s",
                                itm.name.get(), itm.ext.get());
                        continue;
                    }
                    StringBuffer fname;
                    extHandler->getFilename(fname, itm.name);
                    PROGLOG(
                            "Converting legacy external type(%s) to new, file=%s",
                            itm.ext.get(), fname.str());
                    MemoryBuffer mb;
                    ...
                    IExternalHandler *extHandlerNew = queryExternalHandler(
                            EF_BinaryValue);
        ...
        if (items) {
            LOG(
                    MCdebugInfo(100), unknownJob, "Converting %d items larger than threshold size %d, to external definitions", items, externalSizeThreshold);
    ...
    if (remoteBackupLocation.length()) {
        try {
            ...
            LOG(MCoperatorError, unknownJob, e, "Validating backup"); e->Release();}
...
void CCovenSDSManager::saveStore(const char *storeName, bool currentEdition) {
    struct CIgnore {
        CIgnore() {
            SDSManager->ignoreExternals = true;
        }
        ~CIgnore() {
            SDSManager->ignoreExternals = false;
        }
    } ignore;
    iStoreHelper->saveStore(root, NULL, currentEdition);
    unsigned initNodeTableSize = allNodes.maxElements() + OVERFLOWSIZE;
    queryCoven().setInitSDSNodes(
            initNodeTableSize > INIT_NODETABLE_SIZE ?
                    initNodeTableSize : INIT_NODETABLE_SIZE);
}
...
StringBuffer &transformToAbsolute(StringBuffer &result, const char *xpath,
        unsigned index) {
...
void CCovenSDSManager::saveDelta(const char *path, IPropertyTree &changeTree) {
    ...
    if (externalEnvironment) {
        ...
        if (0 == strncmp("/Environment", path, strlen("/Environment"))) {
            WARNLOG("Attempt to change read-only Dali environment, path = %s",
                    path);
            return;
        }
        if (0 == strcmp("/", path)
                && changeTree.hasProp("*[@name=\"Environment\"]")) {
            WARNLOG("Attempt to change read-only Dali environment, path = %s",
                    path);
            return;
        }
    ...
    struct RemoveDIPBlock {
        ...
        RemoveDIPBlock(IFile &_iFile) :
                iFile(_iFile), done(false) {
        }
        ~RemoveDIPBlock() {
            if (!done)
                doit();
    ...
    OwnedIFile detachIPIFile = createIFile(
            detachIPStr.append(DETACHINPROGRESS).str());
    ...
    try {
        ...
        if (remoteBackupLocation.length())
            backupHandler.addDelta(blockedDelta,
                    iStoreHelper->queryCurrentEdition(), first);
        else {
...
CSubscriberContainerList *CCovenSDSManager::getSubscribers(const char *xpath,
        CPTStack &stack) {
...
inline void serverToClientTree(CServerRemoteTree &src, CClientRemoteTree &dst) {
    ...
    if (src.hasChildren())
        dst.addServerTreeInfo(STI_HaveChildren);
}
...
// IMultipleConnector impl.
    virtual void addConnection(const char *xpath, unsigned mode) {
        if (RTM_CREATE == (mode & RTM_CREATE_MASK)
                || RTM_CREATE_QUERY == (mode & RTM_CREATE_MASK))
            throw MakeSDSException(SDSExcpt_BadMode,
                    "multiple connections do not support creation modes");
        xpaths.append(xpath);
        modes.append(mode);
    }
    virtual unsigned queryConnections() {
        return xpaths.ordinality();
    }
    virtual void getConnectionDetails(unsigned which, StringAttr &xpath,
            unsigned &mode) {
...
// ISDSManager impl.
IRemoteConnections *CCovenSDSManager::connect(IMultipleConnector *mConnect,
        SessionId id, unsigned timeout) {
    Owned<CLCLockBlock> lockBlock;
    lockBlock.setown(
            new CLCReadLockBlock(dataRWLock, readWriteTimeout, __FILE__,
                    __LINE__));
...
IRemoteConnection *CCovenSDSManager::connect(const char *xpath, SessionId id,
        unsigned mode, unsigned timeout) {
    Owned<CLCLockBlock> lockBlock;
    Owned<LinkingCriticalBlock> connectCritBlock;
    if (!RTM_MODE(mode, RTM_INTERNAL)) {
        connectCritBlock.setown(
                new LinkingCriticalBlock(connectCrit, __FILE__, __LINE__));
        if (RTM_CREATE == (mode & RTM_CREATE_MASK)
                || RTM_CREATE_QUERY == (mode & RTM_CREATE_MASK))
            lockBlock.setown(
                    new CLCWriteLockBlock(dataRWLock, readWriteTimeout,
                            __FILE__, __LINE__));
        else
            lockBlock.setown(
                    new CLCReadLockBlock(dataRWLock, readWriteTimeout, __FILE__,
    ...
    createConnection(id, mode, timeout, xpath, _tree, connectionId, true,
            connectCritBlock);
    if (connectionId)
        tree.setown(_tree);
    connectCritBlock.clear();
    if (connectionId) {
        CRemoteConnection *conn = new CRemoteConnection(*this, connectionId,
                xpath, id, mode, timeout);
        assertex(conn);
        CDisableFetchChangeBlock block(*conn);
...
SubscriptionId CCovenSDSManager::subscribe(const char *xpath,
        ISDSSubscription &notify, bool sub, bool sendValue) {
    assertex(xpath);
    if (sub && sendValue)
        throw MakeSDSException(SDSExcpt_Unsupported,
                "Subscription to sub elements, with sendValue option unsupported");
    StringBuffer s;
    if ('/' != *xpath) {
        s.append('/').append(xpath);
        xpath = s.str();
    }
    CSDSSubscriberProxy *subscriber = new CSDSSubscriberProxy(xpath, sub,
            sendValue, notify);
    querySubscriptionManager(SDS_PUBLISHER)->add(subscriber,
            subscriber->getId());
    return subscriber->getId();
}

SubscriptionId CCovenSDSManager::subscribeExact(const char *xpath,
        ISDSNodeSubscription &notify, bool sendValue) {
    assertex(xpath);
    StringBuffer s;
    if ('/' != *xpath) {
        s.append('/').append(xpath);
    ...
    CSDSNodeSubscriberProxy *subscriber = new CSDSNodeSubscriberProxy(xpath,
            sendValue, notify);
    querySubscriptionManager(SDSNODE_PUBLISHER)->add(subscriber,
            subscriber->getId());
    return subscriber->getId();
}
...
bool CCovenSDSManager::setSDSDebug(StringArray &params, StringBuffer &reply) {
    if (0 == params.ordinality())
        return false;
    else if (0 == stricmp("datalockHoldTiming", params.item(0))) {
        if (params.ordinality() < 2) {
            reply.append("datalockHoldTiming currently = ").append(
                    readWriteSlowTracing);
            return false;
        }
        unsigned ms = atoi(params.item(1));
        readWriteSlowTracing = ms;

        PROGLOG("datalock, readWriteSlowTracing timing set to %d",
                readWriteSlowTracing);
    } else if (0 == stricmp("datalockHoldStack", params.item(0))) {
        if (params.ordinality() < 2) {
            reply.append("datalockHoldStack currently set to '").append(
                    readWriteStackTracing ? "on" : "off").append("'");
            return false;
        }
        readWriteStackTracing = (0 == stricmp("on", params.item(1)));

        PROGLOG("datalock, held time stacks set to '%s'",
                readWriteStackTracing ? "on" : "off");
    } else if (0 == stricmp("datalockRetryStackTiming", params.item(0))) {
        if (params.ordinality() < 2) {
            reply.append("datalockRetryStackTiming currently =").append(
                    readWriteTimeout);
            return false;
        }
        unsigned ms = atoi(params.item(1));
        ...
        PROGLOG("datalock, readWriteTimeout timing set to %s",
                readWriteStackTracing ? "on" : "off");
...
void CCovenSDSManager::installNotifyHandler(const char *handlerKey,
        ISDSNotifyHandler *handler) {
    nodeNotifyHandlers.replace(
            *new CSDSNotifyHandlerMapping(handlerKey, *handler));
}

// ISDSConnectionManager impl.
void CCovenSDSManager::commit(CRemoteConnection &connection,
        bool *disconnectDeleteRoot) {
    Owned<CLCWriteLockBlock> lockBlock;
    if (!RTM_MODE(connection.queryMode(), RTM_INTERNAL))
        lockBlock.setown(
                new CLCWriteLockBlock(dataRWLock, readWriteTimeout, __FILE__,
                        __LINE__));
    ...
    connection.setLazyFetch(lazyFetch);
    if (NULL == disconnectDeleteRoot && !changeTree)
        return;
    ...
    if (!serverConnection)
        throw MakeSDSException(SDSExcpt_ConnectionAbsent, " [commit]");
    try {
        CServerRemoteTree *serverTree =
                (CServerRemoteTree *) serverConnection->queryRoot();
        assertex(serverTree);
        MemoryBuffer newIds, inc;
        if (changeTree
                && serverTree->processData(*serverConnection, *changeTree,
                        newIds)) { // something commited, if RTM_Create was used need to remember this.
            StringBuffer path;
...
CRemoteTreeBase *CCovenSDSManager::get(CRemoteConnection &connection,
        __int64 serverId) {
    ...
    CRemoteTreeBase *connectionRoot =
            (CRemoteTreeBase *) connection.queryRoot();
    Owned<CServerRemoteTree> tree = getRegisteredTree(
            connectionRoot->queryServerId());
    if (!tree)
        return NULL;
...
void CCovenSDSManager::getChildren(CRemoteTreeBase &parent,
        CRemoteConnection &connection, unsigned levels) {
    Owned<CLCReadLockBlock> lockBlock;
    if (!RTM_MODE(connection.queryMode(), RTM_INTERNAL))
        lockBlock.setown(
                new CLCReadLockBlock(dataRWLock, readWriteTimeout, __FILE__,
                        __LINE__));
    CDisableFetchChangeBlock block(connection);
    Owned<CServerRemoteTree> serverParent =
            (CServerRemoteTree *) getRegisteredTree(parent.queryServerId());
    if (serverParent)
        _getChildren(parent, *serverParent, connection, levels);
}

void CCovenSDSManager::getChildrenFor(CRTArray &childLessList,
        CRemoteConnection &connection, unsigned levels) {
    Owned<CLCReadLockBlock> lockBlock;
    if (!RTM_MODE(connection.queryMode(), RTM_INTERNAL))
        lockBlock.setown(
    ...
    ForEachItemIn(f, childLessList)
    {
        ...
        Owned<CServerRemoteTree> serverParent =
                (CServerRemoteTree *) getRegisteredTree(tree.queryServerId());
        if (serverParent)
            _getChildren(tree, *serverParent, connection, levels);
    }
}

static void addServerChildren(CClientRemoteTree &clientParent,
        CServerRemoteTree &serverParent, bool recurse) {
    Owned<IPropertyTreeIterator> iter = serverParent.getElements("*");
    ForEach (*iter)
    {
        CServerRemoteTree &serverChild = (CServerRemoteTree &) iter->query();
        CClientRemoteTree *clientChild =
                (CClientRemoteTree *) clientParent.create(NULL);
        serverToClientTree(serverChild, *clientChild);
        clientChild = (CClientRemoteTree *) clientParent.addPropTree(
                clientChild->queryName(), clientChild);
        if (recurse)
            addServerChildren(*clientChild, serverChild, recurse);
    }
}

void CCovenSDSManager::matchServerTree(CClientRemoteTree *local,
        IPropertyTree &matchTree, bool allTail) {
    ...
    if (matchIter->first()) {
        if (local->hasChildren() && NULL == local->queryChildren()) {
            ...
            Owned<CServerRemoteTree> tree = getRegisteredTree(
                    matchTree.getPropInt64("@serverId"));
        ...
        do {
            ...
            CClientRemoteTree *child =
                    (CClientRemoteTree *) local->queryPropTree(path.str());
...
void CCovenSDSManager::ensureLocal(CRemoteConnection &connection,
        CRemoteTreeBase &_parent, IPropertyTree *serverMatchTree,
        IPTIteratorCodes flags) {
    CClientRemoteTree &parent = (CClientRemoteTree &) _parent;
    bool getLeaves = iptiter_remotegetbranch
...
void CCovenSDSManager::_getChildren(CRemoteTreeBase &parent,
        CServerRemoteTree &serverParent, CRemoteConnection &connection,
        unsigned levels) {
    Owned<IPropertyTreeIterator> iter = serverParent.getElements("*");
    assertex(iter);
...
IPropertyTreeIterator *CCovenSDSManager::getElements(
        CRemoteConnection &connection, const char *xpath) {
    Owned<CLCReadLockBlock> lockBlock = new CLCReadLockBlock(dataRWLock,
            readWriteTimeout, __FILE__, __LINE__);
    CDisableFetchChangeBlock block(connection);
    Owned<CServerRemoteTree> serverConnRoot =
            (CServerRemoteTree *) getRegisteredTree(
                    ((CClientRemoteTree *) connection.queryRoot())->queryServerId());
...
void CCovenSDSManager::changeMode(CRemoteConnection &connection, unsigned mode,
        unsigned timeout, bool suppressReloads) {
    ...
    CServerRemoteTree *connectionRoot =
            (CServerRemoteTree *) serverConnection->queryRoot();
    ...
    if (!suppressReloads) {
        if (RTM_MODE(mode,
                RTM_LOCK_WRITE) && !RTM_MODE(prevMode, RTM_LOCK_WRITE) && !RTM_MODE(prevMode, RTM_LOCK_READ))
            connection.reload();
    }
}

IPropertyTree *CCovenSDSManager::getXPaths(__int64 serverId, const char *xpath,
        bool getServerIds) {
...
IPropertyTreeIterator *CCovenSDSManager::getXPathsSortLimit(
        const char *baseXPath, const char *matchXPath, const char *sortBy,
        bool caseinsensitive, bool ascending, unsigned from, unsigned limit) {
    Owned<IPropertyTree> matchTree = getXPathsSortLimitMatchTree(baseXPath,
            matchXPath, sortBy, caseinsensitive, ascending, from, limit);
    if (!matchTree)
...
IPropertyTree *CCovenSDSManager::getXPathsSortLimitMatchTree(
        const char *baseXPath, const char *matchXPath, const char *sortby,
        bool caseinsensitive, bool ascending, unsigned from, unsigned limit) {
    UNIMPLEMENTED;
    return NULL;
...
void CCovenSDSManager::getExternalValueFromServerId(__int64 serverId,
        MemoryBuffer &mb) {
    Owned<CServerRemoteTree> idTree =
            (CServerRemoteTree *) SDSManager->getRegisteredTree(serverId);
    if (idTree) {
        CHECKEDCRITICALBLOCK(extCrit, fakeCritTimeout);
        ...
        if (index)
            ...
            WARNLOG(
                    "External file reference missing (node name='%s', id=%" I64F "d)",
                    idTree->queryName(), serverId);
    }
}

IPropertyTreeIterator *CCovenSDSManager::getElementsRaw(const char *xpath,
        INode *remotedali, unsigned timeout) {
...
CServerConnection *CCovenSDSManager::createConnectionInstance(
        CRemoteTreeBase *root, SessionId sessionId, unsigned mode,
        unsigned timeout, const char *xpath, CRemoteTreeBase *&tree,
        ConnectionId _connectionId, StringAttr *deltaPath,
        Owned<IPropertyTree> &deltaChange, Owned<CBranchChange> &branchChange,
        unsigned &additions) {
    ...
    if (mode & RTM_CREATE) {
        ...
        if (head.length()) {
            try {
                parent = createPropBranch(root, head.str(), true, &created,
                        &createdParent);
                if (created)
                    newNode = true;
            } catch (IException *) {
                if (created)
                    createdParent->removeTree(created);
                throw;
            } catch (DALI_CATCHALL) {
                if (created)
                    createdParent->removeTree(created);
        ...
        if (parent) {
            ...
            if (RTM_MODE(mode, RTM_CREATE_QUERY))
                tree = (CRemoteTreeBase *) root->queryCreateBranch(parent, prop,
                        &newNode);
            else {
                IPropertyTree *newTree = ((CRemoteTreeBase *) parent)->create(
                        NULL);
                if (RTM_MODE(mode, RTM_CREATE_ADD))
                    tree = (CRemoteTreeBase *) parent->addPropTree(prop,
                            newTree);
                else // Default - RTM_CREATE - replace existing
                {
                    ...
                    if (iter->first())
                        replaceNode = true;
                    tree = (CRemoteTreeBase *) parent->setPropTree(prop,
                            newTree);
                }
                newNode = true;
            }
        ...
        if (NULL == _xpath || '\0' == *_xpath)
            ...
            ForEach (*iter)
            {
                ...
                if (tree) {
                    if (match)
                        throw MakeSDSException(SDSExcpt_AmbiguousXpath,
                                "Ambiguous: %s", _xpath);
                } else {
                    parent = _parent;
    ...
    CServerConnection *connection = new CServerConnection(*this, connectionId,
            _xpath, sessionId, mode, timeout, parent, connInfoFlags);
    ...
    if (newNode) {
        ...
        if (created) // some elems in "head" created
        {
            ...
            branchChange.setown(
                    new CBranchChange(*(CRemoteTreeBase *) createdParent));
            ...
            for (; s < connection->queryPTreePath().ordinality(); s++) {
                ...
                IPropertyTree &tree = connection->queryPTreePath().item(s);
                IPropertyTree *n = tail->addPropTree(RESERVED_CHANGE_NODE,
                        createPTree());
...
void CCovenSDSManager::changeLockMode(CServerConnection &connection,
        unsigned newMode, unsigned timeout) {
    ...
    CUnlockCallback callback(connection.queryXPath(), connectionId, *tree);
    {
        ...
        if (lock) {
            lock->changeMode(connectionId, connection.querySessionId(), newMode,
                    timeout, callback);
    ...
    lock(*tree, connection.queryXPath(), connectionId,
            connection.querySessionId(), newMode, timeout, callback);
    connection.setMode(newMode);
}

bool CCovenSDSManager::unlock(__int64 connectionId, bool close,
        StringBuffer &connectionInfo) {
    ...
    if (close) {
        PROGLOG("forcing unlock & disconnection of connection : %s",
                connectionInfo.str());
        Owned<CLCLockBlock> lockBlock = new CLCWriteLockBlock(dataRWLock,
                readWriteTimeout, __FILE__, __LINE__);
        SDSManager->disconnect(connectionId, false);
    } else // leave connection open, just unlock
    {
        ...
        __int64 nodeId =
                ((CRemoteTreeBase *) connection->queryRoot())->queryServerId();
        CHECKEDCRITICALBLOCK(lockCrit, fakeCritTimeout);
...
LockStatus CCovenSDSManager::establishLock(CLock &lock, __int64 treeId,
        ConnectionId connectionId, SessionId sessionId, unsigned mode,
        unsigned timeout, IUnlockCallback &lockCallback) {
    LockStatus res = lock.lock(mode, timeout, connectionId, sessionId,
            lockCallback);
...
void CCovenSDSManager::lock(CServerRemoteTree &tree, const char *xpath,
        ConnectionId connectionId, SessionId sessionId, unsigned mode,
        unsigned timeout, IUnlockCallback &callback) {
    ...
    if (!lock) {
        IdPath idPath;
        lock = new CLock(lockTable, treeId, idPath, xpath, mode, connectionId,
                sessionId);
        lockTable.replace(*lock);
    } else {
        Linked<CLock> tmp = lock; // keep it alive could be destroyed whilst blocked in call below.
        LockStatus result = establishLock(*lock, treeId, connectionId,
                sessionId, mode, timeout, callback);
        if (result != LockSucceeded) {
            ...
            case LockFailed:
                throw MakeSDSException(SDSExcpt_ConnectionAbsent,
                        "Lost connection trying to establish lock on connection to : %s",
                        xpath);
            case LockTimedOut:
                throw MakeSDSException(SDSExcpt_LockTimeout,
                        "Lock timeout trying to establish lock to %s, existing lock info: %s",
                        xpath, lock->toString(s).str());
            case LockHeld:
                throw MakeSDSException(SDSExcpt_LockHeld,
                        "Lock is held trying to establish lock to %s, existing lock info: %s",
                        xpath, lock->toString(s).str());
            }
        }
    }
...
void CCovenSDSManager::createConnection(SessionId sessionId, unsigned mode,
        unsigned timeout, const char *xpath, CServerRemoteTree *&tree,
        ConnectionId &connectionId, bool primary,
        Owned<LinkingCriticalBlock> &connectCritBlock) {
    ...
    struct CFreeExistingLocks {
        ...
        void clear() {
            ForEachItemIn(l, existingLockTrees)
                SDSManager->unlock(existingLockTrees.item(l).queryServerId(),
                        connId);
            existingLockTrees.kill();
        }
        void setConnectionId(ConnectionId _connId) {
            connId = _connId;
        }
        ...
        bool isExisting(CServerRemoteTree &tree) {
            return NotFound != existingLockTrees.find(tree);
        }
        ConnectionId connId;
    ...
    try {
        struct LockUnblock {
            LockUnblock(ReadWriteLock &_rWLock) :
                    rWLock(_rWLock) {
        ...
        class CConnectExistingLockCallback: implements IUnlockCallback {
            ...
            CheckedCriticalSection *connectCrit;
        public:
            CConnectExistingLockCallback(const char *xpath,
                    ConnectionId connectionId, CServerRemoteTree &tree,
                    CheckedCriticalSection *_connectCrit) :
                    lcb(xpath, connectionId, tree), connectCrit(_connectCrit) {
            }
            virtual void block() {
                if (connectCrit)
        ...
        if (!RTM_MODE(mode,
                RTM_CREATE_ADD) && !RTM_MODE(mode, RTM_CREATE_UNIQUE)) // cannot be pending on existing locked nodes in these cases
                {
            CTimeMon tm(timeout);
            connectionId = coven.getUniqueId();
            Owned<CServerConnection> tmpConn = new CServerConnection(*this,
                    connectionId, xpath, sessionId, mode, timeout, NULL,
                    (ConnInfoFlags) 0);
            {
            ...
            try {
                loop {
                    try {
                        ...
                        iter->first();
                        while (iter->isValid()) {
                            CServerRemoteTree &existing =
                                    (CServerRemoteTree &) iter->query();
                            if (freeExistingLocks.isExisting(existing))
                                ...
                                freeExistingLocks.add(existing);
                                {
                                    ...
                                    if (timeout) {
                                        if (tm.timedout(&remaining)) {
                                            Linked<CLock> lock;
                                            VStringBuffer timeoutMsg(
                                                    "Failed to establish lock to %s, timeout whilst retrying connection to orphaned connection path",
                                                    xpath);
                                            ForEachItemIn(f,
                                                    freeExistingLocks.existingLockTrees) {
                                                CServerRemoteTree &e =
                                                        freeExistingLocks.existingLockTrees.item(
                                                                f);
                                                {
                                                    CHECKEDCRITICALBLOCK(
                                                            lockCrit,
                                                            fakeCritTimeout);
                                                    CLock *_lock = queryLock(
                                                            e.queryServerId());
                                                    if (_lock) {
                                                        if (!lock)
                                                            timeoutMsg.append(
                                            ...
                                            throw MakeSDSException(
                                                    SDSExcpt_LockTimeout, "%s",
                                                    timeoutMsg.str());
                                        }
                                    } else
                                        remaining = 0; // a timeout of 0 means fail immediately if locked
                                    CConnectExistingLockCallback connectLockCallback(
                                            xpath, connectionId, existing,
                                            &connectCrit);
                                    lock(existing, xpath, connectionId,
                                            sessionId, mode, remaining,
                                            connectLockCallback);
                                }
                                if (!queryConnection(connectionId)) // aborted
                                        {
        ...
        try {
            if (RTM_MODE(mode,
                    RTM_CREATE_ADD) || RTM_MODE(mode, RTM_CREATE_UNIQUE)) {
                CHECKEDCRITICALBLOCK(blockedSaveCrit, fakeCritTimeout);
                connection.setown(
                        createConnectionInstance(root, sessionId, mode, timeout,
                                xpath, _tree, connectionId, &deltaPath,
                                deltaChange, branchChange, additions));
            } else
                connection.setown(
                        createConnectionInstance(root, sessionId, mode, timeout,
                                xpath, _tree, connectionId, &deltaPath,
        ...
        } catch (IException *) // do not want to miss recording change to inc, under any circumstances.
        {
            if (deltaChange.get()) {
                PROGLOG(
                        "Exception on RTM_CREATE caused call to saveDelta, xpath=%s",
                        xpath);
        ...
        try {
            if (!locked) {
                CConnectExistingLockCallback connectLockCallback(xpath,
                        connectionId, *(CServerRemoteTree *) _tree,
                        connectCritBlock.get() ? &connectCrit : NULL);
                lock(*(CServerRemoteTree *) _tree, xpath, connectionId,
                        sessionId, mode, timeout, connectLockCallback);
            }
        } catch (IException *) {
            {
                CHECKEDCRITICALBLOCK(cTableCrit, fakeCritTimeout);
...
void CCovenSDSManager::disconnect(ConnectionId id, bool deleteRoot,
        Owned<CLCLockBlock> *lockBlock) {
    ...
    Linked<CServerRemoteTree> tree =
            (CServerRemoteTree *) connection->queryRootUnvalidated();
    if (!tree)
        return;
    ...
    if (connection->queryParent()) {
        if (deleteRoot
                || RTM_MODE(connection->queryMode(), RTM_DELETE_ON_DISCONNECT)) {
            CHECKEDCRITICALBLOCK(lockCrit, fakeCritTimeout);
            CLock *lock = queryLock(tree->queryServerId());
            if (lock) {
                deleteRoot = false;
                lock->setDROLR((CServerRemoteTree *) connection->queryParent(),
                        tree);
            } else
                deleteRoot = true;
        }
        if ((unsigned) -1 == index)
            index = connection->queryParent()->queryChildIndex(
                    connection->queryRootUnvalidated());
    } else
        deleteRoot = false;

    bool orphaned =
            ((CServerRemoteTree*) connection->queryRootUnvalidated())->isOrphaned();
    ...
    if (deleteRoot) {
        ...
        if (!orphaned) {
            ...
            IPropertyTree *d = changeTree->setPropTree(DELETE_TAG,
                    createPTree());
            ...
            if (!RTM_MODE(connection->queryMode(), RTM_INTERNAL)) {
                connection->notify();
                SDSManager->startNotification(*changeTree, stack,
                        *branchChange);
            }
            ...
            else { // NB: don't believe this can happen, but last thing want to do is save duff delete delta.
                WARNLOG(
                        "** CCovenSDSManager::disconnect - index position lost **");
                PrintStackReport();
            }
        }
    }
...
StringBuffer &formatConnectionInfo(MemoryBuffer &src, StringBuffer &out) {
    ...
    unsigned timeout;
    bool established;
    src.read(connectionId).read(xpath).read(sessionId).read(mode).read(timeout).read(
            established);
    out.append("ConnectionId=").appendf("%" I64F "x", connectionId).append(
            ", xpath=").append(xpath).append(", sessionId=").appendf(
            "%" I64F "x", sessionId).append(", mode=").append(mode).append(
            ", timeout=");
    if (INFINITE == timeout)
...
StringBuffer &formatSubscriberInfo(MemoryBuffer &src, StringBuffer &out) {
    ...
    out.append("SubscriptionId=").appendf("%" I64F "x", subscriptionId).append(
            ", xpath=").append(xpath).append(", sub=").append(
            sub ? "true" : "false");
    return out;
}
...
StringBuffer &formatNodeSubscriberInfo(MemoryBuffer &src, StringBuffer &out) {
    ...
    out.append("SubscriptionId=").appendf("%" I64F "x", subscriptionId).append(
            ", xpath=").append(xpath).append(", nodes=").append(nodeCount);
    return out;
}
...
ILockInfoCollection *CCovenSDSManager::getLocks(const char *ipPattern,
        const char *xpathPattern) {
    ...
    CLockInfoArray locks;
    {
        ...
        ForEach(iter)
        {
            ...
            if (lock.lockCount()) {
                if (!filteredXPaths
                        || WildMatch(lock.queryXPath(), xpathPattern))
...
MemoryBuffer &CCovenSDSManager::collectSubscribers(MemoryBuffer &out) {
    ...
    out.append(subscribers.count());
    SuperHashIteratorOf<CSubscriberContainer> sdsIter(
            subscribers.queryBaseTable());
    ForEach(sdsIter)
...
bool CCovenSDSManager::updateEnvironment(IPropertyTree *newEnv,
        bool forceGroupUpdate, StringBuffer &response) {
    Owned<IRemoteConnection> conn = querySDS().connect("/", myProcessSession(),
            0, INFINITE);
    if (conn) {
        ...
        if (oldEnvironment.get()) {
            ...
            Owned<IFileIO> io = createUniqueFile(NULL, "environment", "bak",
                    bakname);
...
void CCovenSDSManager::handleNodeNotify(notifications n,
        CServerRemoteTree &tree) {
    ...
    if (!m) {
        LOG(
                MCwarning, unknownJob, "Unknown notify handler name \"%s\", handing event %s", handlerKey, notificationStr(n));
        return;
    }
    switch (n) {
    case notify_delete:
        try {
            m->query().removed(tree);
        } catch (IException *e) {
            StringBuffer s(
                    "Exception calling ISDSNotifyHandler->removed(<tree>), for handler\"");
...
void CCovenSDSManager::handleNotify(CSubscriberContainerBase *_subscriber,
        MemoryBuffer &notifyData) {
    ...
    if (!notifyPool) {
        ...
        notifyPool.setown(
                createThreadPool("SDS Notification Pool", factory, this,
                        SUBNTFY_POOL_SIZE));
        factory->Release();
    }
    ...
    if (notifier) {
        ...
        Owned<CSubscriberNotifier> _notifier = new CSubscriberNotifier(
                subscriberNotificationTable, *subscriber.getClear(),
                notifyData);
...
public:
    struct PushPop {
        PushPop(CPTStack &_stack, CRemoteTreeBase &tree) :
                stack(_stack) {
            tree.Link();
            stack.append(tree);
        }
    ...
    CSubscriberNotifyScanner(IPropertyTree &_changeTree, CPTStack &_stack,
            CBranchChange &_rootChanges) :
            changeTree(&_changeTree), rootChanges(&_rootChanges), stack(_stack) {
        INIT_NAMEDCOUNT;
        SDSManager->querySubscriberTable().getSubscribers(subs);
    }
    enum SubCommitType {
        subCommitNone, subCommitExact, subCommitBelow, subCommitAbove
    };
    ...
    void scan() {
        ...
        if (stack.ordinality() && (rootChanges->tree == &(stack.tos())))
            stack.pop();
        CSubscriberArray pruned;
        scan(*rootChanges, stack, pruned);
    }

    bool prune(const char *xpath, CSubscriberCopyArray &candidates,
            CSubscriberArray &pruned) {
    ...
    void scanAll(PDState state, CBranchChange &changes, CPTStack &stack,
            CSubscriberArray &pruned) {
            ...
            if (candidates.ordinality()) {
                    ...
                    if (!subscriber.isUnsubscribed()) {
                        if (subscriber.qualify(stack, true)) {
                            if (0 == notifyData.length())
                                buildNotifyData(notifyData, state, &stack,
                                        NULL);
                            SDSManager->handleNotify(LINK(&subscriber),
                                    notifyData);
                        } else
                            pruned.append(*LINK(&subscriber));
                    }
                ...
                if (0 == changes.children.ordinality()) {
                    ...
                    ForEachItemIn(c, changes.children)
                    {
                        ...
                        ForEachItemIn(i, _pruned)
                            subs.append(*LINK(&_pruned.item(i)));
    ...
    void scan(CBranchChange &changes, CPTStack &stack,
            CSubscriberArray &pruned) {
        CSubscriberCopyArray candidates;
        if (!prune(xpath.str(), candidates, pruned))
            return;
                        ...
                        if (subscriber.qualify(stack, false)) {
                            if (subscriber.querySendValue()) {
                                if (1 != lastSendValue) {
                                    ...
                                    buildNotifyData(notifyData.clear(),
                                            changes.state, &stack, &mb);
                                    lastSendValue = 1;
                                }
                            } else {
                                if (0 != lastSendValue) {
                                    buildNotifyData(notifyData.clear(),
                                            changes.state, &stack, NULL);
                                    lastSendValue = 0;
                                }
                            }
                            SDSManager->handleNotify(LINK(&subscriber),
        ...
        ForEachItemIn(c, changes.children)
        {
            ...
            ForEachItemIn(i, pruned)
                subs.append(*LINK(&pruned.item(i)));
            if (0 == subs.ordinality())
                break;
            xpath.setLength(parentLength);
        }
...
void CCovenSDSManager::startNotification(IPropertyTree &changeTree,
        CPTStack &stack, CBranchChange &changes) {
    ...
    if (!scanNotifyPool) {
        ...
        scanNotifyPool.setown(
                createThreadPool("SDS Scan-Notification Pool", factory, this,
                        SUBSCAN_POOL_SIZE));
        factory->Release();
    }

    Owned<CSubscriberNotifyScanner> scan = new CSubscriberNotifyScanner(
            changeTree, stack, changes);
    scanNotifyPool->start(scan.get());
}

void CCovenSDSManager::deleteExternal(__int64 index) {
    if (server.queryStopped())
        return;
    CHECKEDCRITICALBLOCK(extCrit, fakeCritTimeout);
    IExternalHandler *handler = queryExternalHandler(EF_BinaryValue);
    assertex(handler);
...
void CCovenSDSManager::serializeExternal(__int64 index, IPropertyTree &owner,
        MemoryBuffer &mb, bool withValue) {
    CHECKEDCRITICALBLOCK(extCrit, fakeCritTimeout);
    IExternalHandler *extHandler = queryExternalHandler(EF_BinaryValue);
    assertex(extHandler);
...
void CCovenSDSManager::writeExternal(CServerRemoteTree &tree, bool direct,
        __int64 existing) {
    CHECKEDCRITICALBLOCK(extCrit, fakeCritTimeout);
    __int64 index;
    if (existing)
...
void CCovenSDSManager::addNodeSubscriber(ISubscription *sub,
        SubscriptionId id) {
    MemoryBuffer mb;
    mb.setBuffer(sub->queryData().length(), (void *) sub->queryData().get());
    StringAttr xpath;
    ...
    if (!iter->first())
        throw MakeSDSException(SDSExcpt_SubscriptionNoMatch,
                "Failed to match any nodes: %s", xpath.get());
    else {
        Owned<CNodeSubscriberContainer> subscriber =
                new CNodeSubscriberContainer(sub, id, sendValue, xpath);
        do {
...
public:
    ...
    CDaliSDSServer(IPropertyTree *_config) :
            config(_config) {
        manager = NULL;
        cancelLoad = false;
        storeLoaded = false;
    ...
    void start() {
        ...
        manager = new CCovenSDSManager(coven, *sdsConfig,
                config ? config->queryProp("@dataPath") : NULL);
        SDSManager = manager;
        addThreadExceptionHandler(manager);
        try {
            manager->loadStore(NULL, &cancelLoad);
        } catch (IException *) {
...
bool applyXmlDeltas(IPropertyTree &root, IIOStream &stream, bool stopOnError) {
    ...
    public:
        ...
        CDeltaProcessor(IPropertyTree &_store, bool _stopOnError) :
                store(_store), stopOnError(_stopOnError), level(0) {
            sectionEndOffset = 0;
            hadError = false;
        ...
        void apply(IPropertyTree &change, IPropertyTree &currentBranch) {
            if (change.getPropBool("@localValue")) {
                ...
                if (binary) {
                    ...
                    currentBranch.setPropBin(NULL, mb.length(),
                            mb.toByteArray());
                } else
                ...
                if (change.queryProp(NULL)) {
                    ...
                    MemoryBuffer mb;
                    if (index
                            && QUERYINTERFACE(&currentBranch, CServerRemoteTree)) {
                        MemoryBuffer mbv;
                        SDSManager->getExternalValue(index, mbv);
                        CPTValue v(mbv);
                        v.getValue(mb, binary);
                    ...
                    if (binary)
                        ...
                        currentBranch.setProp(NULL,
                                (const char *) mb.toByteArray());
                }
            }
            ...
            ForEach (*iter)
            {
                ...
                verifyex(
                        currentBranch.renameProp(xpath.str(),
                                d.queryProp("@to")));
            }
            ...
            ForEach (*iter)
            {
                ...
                if (!currentBranch.removeProp(xpath.str()))
                    LOG(
                            MCoperatorWarning, unknownJob, "Property '%s' missing, but recorded as being present at time of delete, in section '%s'", xpath.str(), headerPath.get());
                }
            IPropertyTree *ac = change.queryPropTree(ATTRCHANGE_TAG);
            if (ac) {
                Owned<IAttributeIterator> aIter = ac->getAttributes();
                ForEach (*aIter)
                    currentBranch.setProp(aIter->queryName(),
                            aIter->queryValue());
            }
            ...
            if (ad) {
                ...
                ForEach (*aIter)
                {
                    if (!currentBranch.removeProp(aIter->queryName()))
                        LOG(
                                MCoperatorWarning, unknownJob, "Property '%s' missing, but recorded as being present at time of delete, in section '%s'", aIter->queryName(), headerPath.get());
                    }
                }
        ...
        void processChildren(IPropertyTree &change,
                IPropertyTree &currentBranch) {
            // process children
            ...
            ForEach (*iter)
            {
                try {
                    ...
                    if (child.getPropBool("@new")) {
                        IPropertyTree *newBranch = currentBranch.addPropTree(
                                name, createPTree());
                        apply(child, *newBranch);
                    } else if (child.getPropBool("@replace")) {
                        IPropertyTree *newBranch = currentBranch.setPropTree(
                                name, createPTree());
                        apply(child, *newBranch);
                    } else {
                        ...
                        if (!pos)
                            throw MakeStringException(0,
                                    "Missing position attribute in child reference, section end offset=%" I64F "d",
                                    sectionEndOffset);
                        StringBuffer xpath(name);
                        xpath.append('[').append(pos).append(']');
                        IPropertyTree *existingBranch =
                                currentBranch.queryPropTree(xpath.str());
                        if (!existingBranch)
                            throw MakeStringException(0,
                                    "Failed to locate delta change in %s, section end offset=%" I64F "d",
                                    xpath.str(), sectionEndOffset);
                        apply(child, *existingBranch);
                    }
                } catch (IException *e) {
                    StringBuffer s(
                            "Error processing delta section: sectionEndOffset=");
                    LOG(
                            MCoperatorError, unknownJob, e, s.append(sectionEndOffset).str());
                    if (stopOnError)
                        throw;
                    hadError = true;
                    e->Release();
        ...
        void process(IPropertyTree &match, offset_t endOffset) {
            ...
            if (!root)
                throw MakeStringException(0,
                        "Failed to locate header xpath = %s", xpath);
            IPropertyTree *start = match.queryPropTree("Delta/T");
            if (!start)
                throw MakeStringException(0,
                        "Badly constructed delta format (missing Delta/T) in header path=%s, section end offset=%" I64F "d",
                        xpath, endOffset);
    ...
    Owned<IPullPTreeReader> xmlReader = createPullXMLStreamReader(stream,
            deltaProcessor,
            (PTreeReaderOptions) ((unsigned) ptr_ignoreWhiteSpace
                    + (unsigned) ptr_noRoot), false);
    try {
        xmlReader->load();
    } catch (IException *e) {
        if (stopOnError)
            throw;
        LOG(
                MCoperatorError, unknownJob, e, "XML parse error on delta load - load truncated");
        e->Release();
    }
...
void LogRemoteConn(IRemoteConnection *conn) {
    ...
    IPropertyTree *root = conn->queryRoot();
    CRemoteTreeBase *remotetree =
            root ? QUERYINTERFACE(root, CRemoteTreeBase) : NULL;
    unsigned rcount =
            remotetree ? remotetree->getLinkCount() - 1 : ((unsigned) -1);
    PROGLOG("CONN(%x,%" I64F "x,%" I64F "x) path = '%s' mode = %x, link %d,%d",
            (unsigned) (memsize_t) conn, (__int64) conbase->querySessionId(),
            (__int64) conbase->queryConnectionId(), conbase->queryXPath(),
            conbase->queryMode(), conbase->getLinkCount() - 1, rcount);
}

#ifdef _POOLED_SERVER_REMOTE_TREE

MODULE_INIT(INIT_PRIORITY_STANDARD) {
    CServerRemoteTree_Allocator = new CFixedSizeAllocator(
            sizeof(CServerRemoteTree));
    return true;
}
MODULE_EXIT()
{
