...
bool readSlaveLogPattern(const char *pattern, unsigned &numberOfSlaveLogs, StringArray &logDates)
{
    ...
    const char *mPtr = strstr(tail, OFSTR);
    if (!mPtr)
        return false; //Should not happen

    mPtr += strlen(OFSTR);
    const char *dPtr = strchr(mPtr, '.');
    if (!dPtr)
        return false; //Should not happen

    StringBuffer numberOfSlaveLogsStr;
    numberOfSlaveLogsStr.append(dPtr - mPtr, mPtr);
    ...
    const char *extPtr = strchr(++dPtr, '.');
    if (!extPtr)
        return false; //Should not happen

    StringBuffer numberOfDaysStr;
    numberOfDaysStr.append(extPtr - dPtr, dPtr);
